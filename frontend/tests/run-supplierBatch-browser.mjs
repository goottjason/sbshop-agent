...
const fixture = process.argv[2] ?? 'supplierBatch.browser.tsx';
if (!/^[A-Za-z0-9._-]+\.browser\.tsx$/.test(fixture)) throw new Error('Expected a local browser fixture filename');
const out = await mkdtemp(join(tmpdir(), 'sbshop-supplier-batch-browser-'));
...
const capture = process.env.SBSHOP_BATCH_CAPTURE === 'drawer' ? 'drawer' : 'main';
const html = `<!doctype html><html lang="ko"><meta charset="utf-8"><title>소싱처 전체 배치 검증 · 로컬 fixture</title><style>*{box-sizing:border-box}body{margin:0;background:#f4f6f8;font-family:Arial,sans-serif}.product-theme{--product-primary:#166534}${css}</style><body><div id="root"></div><script>window.__batchCapture=${JSON.stringify(capture)};</script><script>${js.replaceAll('</script', '<\\/script')}</script></body></html>`;
...
({ stdout } = await promisify(execFile)(chrome, ['--headless', '--disable-gpu', '--no-first-run', '--no-default-browser-check',
  ...
  `--user-data-dir=${join(out, 'chrome-profile')}`, capture === 'drawer' ? '--window-size=1440,1100' : '--window-size=1440,900', '--virtual-time-budget=18000', '--dump-dom',
...
await writeFile(join(out, 'dom.html'), stdout);
...
result.fixtureOnly = true; result.capture = capture;
