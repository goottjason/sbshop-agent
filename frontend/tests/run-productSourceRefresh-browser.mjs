...
const fixture = process.argv[2] ?? 'productSourceRefresh.browser.tsx';
if (!/^[A-Za-z0-9._-]+\.browser\.tsx$/.test(fixture)) throw new Error('Expected a local browser fixture filename');
const out = await mkdtemp(join(tmpdir(), 'sbshop-source-browser-'));
...
const html = `<!doctype html><html lang="ko"><meta charset="utf-8"><title>상품 콘텐츠 비교 검증 · fixture</title><style>*{box-sizing:border-box}body{margin:0;background:#f4f6f8;font-family:Arial,sans-serif}.product-theme{--product-primary:#166534}${css}</style><body><div id="root"></div><script>${js.replaceAll('</script', '<\\/script')}</script></body></html>`;
...
// The source price/stock fixture has no image downloads.
