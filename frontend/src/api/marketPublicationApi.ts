...
import type { PublicationInputContext } from './productPublicationInputsApi';
...
export interface RegistrationTask {
  ...
  listingId: string | null; expiresAt: string; createdAt: string; checkedAt: string | null; committed: boolean; shippingSummary?: Record<string, string>;
}
...
export const marketPublicationApi = {
  ...
  prepare: (selected: { productId: number; market: string; context?: PublicationInputContext }[]) => apiClient.post<{ prepared: RegistrationTask[]; excluded: RegistrationCandidate[] }>(`${root}/reviews`, { selected }),
