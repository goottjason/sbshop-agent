  ...
  const renderRun = (listRun: SupplierBatchRun) => {
    ...
    return <article key={run.id} className={`sb-batch-run ${expanded ? 'sb-batch-run-open' : ''}`}>
      <div className="sb-batch-run-header"><button className="sb-batch-run-toggle" type="button" aria-expanded={expanded} aria-controls={`run-${run.id}`} onClick={() => setExpandedId(expanded ? null : run.id)}>
        ...
        <span className="sb-batch-run-progress"><span>처리 {numberText(run.processed)} / {numberText(run.total)}개</span><Progress percent={percent} showInfo={false} size="small" strokeColor={BATCH_BLUE} status="normal" /></span>
        <span className="sb-batch-run-counts"><span>성공 <b>{numberText(run.succeeded)}</b></span><span className={run.failed ? 'sb-batch-error-text' : ''}>실패 <b>{numberText(run.failed)}</b></span><span>보류 <b>{numberText(run.blocked)}</b></span><span>대기·처리 <b>{numberText(run.pending)}</b></span></span>
