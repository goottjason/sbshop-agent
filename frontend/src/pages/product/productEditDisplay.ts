...
const labels: Record<string, string> = {
  ...
  couponRate: '쿠폰율', minMarginPrice: '최소마진', salesQuantity: '판매용 설정 수량', stockStatus: '소싱 재고 상태', stock: 'DB 재고', weight: '무게(kg)', bundleQuantity: '묶음수량',
...
export const editValue = (field: string, value: string | null) => value === null ? '값 없음' : field === 'stockStatus' ? ({ IN_STOCK: '재고 있음', OUT_OF_STOCK: '품절' }[value] ?? value) : numericFields.has(field) ? formatNumericPreviewValue(value) : value || '(비어 있음)';
