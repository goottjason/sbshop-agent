...
import { Alert, Button, Checkbox, Input, Modal, Table, Tag } from 'antd';
...
const stateLabels: Record<string, string> = { PREVIEW: '등록 전 검토', QUEUED: '등록 전송 대기', POST_STARTED: '등록 전송 중', VERIFY: '생성 상품 재조회 대기', UNKNOWN_CREATE: '생성 여부 미확인', ACTION_REQUIRED: '결과 확인 필요', REGISTERED: '등록·주요 정보 확인', STALE: '다시 검토 필요' };
...
export function ProductRegistrationJobs({ productIds, onClose }: { productIds: number[]; onClose: () => void }) {
  ...
  const loadCandidates = async () => {
    setBusy(true); setError(null); setCandidates([]); setSelected([]); setPrepared([]); setToCommit([]); setReviewed(false);
  ...
  const prepare = async () => {
    ...
    try {
      const result = (await marketPublicationApi.prepare(candidates.filter(c => selected.includes(candidateKey(c)) && c.selectable).map(c => ({ productId: c.productId, market: c.market })))).data;
  ...
  return <Modal open title="미등록·삭제 상품 등록" width={1180} footer={null} maskClosable={!busy} closable={!busy} onCancel={() => { if (!busy) onClose(); }}>
    <Alert type="info" showIcon message="삭제 사유와 새 등록 내용을 확인한 상품만 선택해 등록합니다."
      description="스마트스토어의 검토·등록·재조회 흐름을 제공합니다. 영구 판매금지와 생성 여부 미확인 상품은 제외합니다. 나머지 마켓은 등록 요청과 결과 검증 연결을 준비 중입니다." />
    ...
    {!!candidates.length && <>
      <Table<RegistrationCandidate> size="small" rowKey={candidateKey} dataSource={candidates} scroll={{ x: 850, y: 300 }} pagination={{ pageSize: 12, showSizeChanger: false }}
        rowSelection={{ selectedRowKeys: selected, onChange: keys => setSelected(keys), getCheckboxProps: r => ({ disabled: busy || !r.selectable }) }} columns={[
      ...
      <Button loading={busy} disabled={!selected.length || selected.length > 100} onClick={() => { void prepare(); }}>선택 후보 {selected.length}건 등록 내용 준비</Button>
    </>}
    {!!prepared.length && <>
      ...
      <p><Checkbox checked={reviewed} disabled={busy} onChange={e => setReviewed(e.target.checked)}>삭제 사유와 새 상품명·카테고리·가격·수량을 확인했습니다.</Checkbox></p>
    ...
    <Modal open={!!resolve} title="생성된 원상품 번호로 확인" onCancel={() => { if (!busy) setResolve(null); }} onOk={() => { void recheck(); }} confirmLoading={busy} okText="원상품 재조회 접수" okButtonProps={{ disabled: !/^[1-9][0-9]{0,17}$/.test(listingId.trim()) }}>
      <p>{resolve?.sbCode} · 등록을 요청한 스마트스토어 계정에서 생성된 원상품 번호를 입력하세요. 과거 삭제 상품번호는 사용할 수 없습니다. SB코드와 검토한 주요 정보가 일치할 때 연결을 확정합니다.</p>
