...
import {
  productSourceApi, type ProductSourceCollectionRequest, type ProductSourceField,
  type ProductSourceSnapshot, type ProductSourceValues,
} from '../../api/productSourceApi';
...
const RECOVERY_KEY = 'sbshop.productSource.collection';
const fieldLabels: Record<ProductSourceField, string> = { PRICE: '매입 원가·환율', STOCK: '소싱 재고 상태·실재고' };
const stateLabels: Record<ProductSourceSnapshot['state'], string> = {
...
const collecting = (item: ProductSourceSnapshot) => ['QUEUED', 'COLLECTING'].includes(item.state);

interface Recovery { request: ProductSourceCollectionRequest; collectionId?: string }
...
function SourceValues({ content, field }: { content: ProductSourceValues | null; field: ProductSourceField }) {
  if (!content) return <p className="pw-content-empty">수집된 값 없음</p>;
  const number = (value: number | null) => value == null ? '관측 없음 · 기존값 유지' : value.toLocaleString('ko-KR');
  return field === 'PRICE' ? <><p>개당 매입 원가 {number(content.costPrice)}{content.costPrice != null && '원'}</p>
    <p>환율 {number(content.exchangeRate)}</p></> : <><p>재고 상태 {content.stockStatus === 'IN_STOCK' ? '재고 있음' : content.stockStatus === 'OUT_OF_STOCK' ? '품절' : '관측 없음 · 기존값 유지'}</p>
    <p>소싱 실재고 {number(content.stock)}{content.stock != null && '개'}</p></>;
}

function SnapshotCard({ snapshot, selected, disabled, initiallyExpanded, onSelection }: {
  snapshot: ProductSourceSnapshot; selected: ProductSourceField[]; disabled: boolean;
      ...
      <details className="pw-content-comparison" open={expanded} onToggle={event => setExpanded(event.currentTarget.open)}><summary>비교 및 적용 항목 선택</summary>{expanded && snapshot.fields.map(rule => <section key={rule.field} className="pw-content-field">
        ...
        <details className="pw-content-comparison" open={rule.available || undefined}>
          ...
          <div className="pw-content-compare-grid">
            <div><h4>수집 요청 당시 DB</h4><SourceValues content={snapshot.current} field={rule.field} /></div>
            <div><h4>이번에 수집한 내용{!rule.available && ' · 적용 불가'}</h4><SourceValues content={snapshot.proposed} field={rule.field} /></div>
          </div>
        </details>
      </section>)}</details>}
  </article>;
...
export function ProductSourceRefreshModal({ productIds, onClose, onSaved }: {
  ...
  const collection = useQuery({ queryKey: ['product-source-collection', collectionId], enabled: !!collectionId,
    queryFn: async ({ signal }) => (await productSourceApi.collection(collectionId!, signal)).data,
    retry: false, refetchInterval: query => query.state.data?.items.some(collecting) ? 5000 : false });
  const history = useQuery({ queryKey: ['product-source-history', productIds[0]], enabled: productIds.length === 1,
  ...
  const collect = async (retry?: ProductSourceCollectionRequest) => {
    ...
    try {
      const result = (await productSourceApi.collect(request)).data;
      const saved = { request, collectionId: result.id }; persistRecovery(saved); setRecovery(saved);
      queryClient.setQueryData(['product-source-collection', result.id], result);
  ...
  const prepare = async () => {
    ...
    try { setReview((await productSourceApi.review(selected)).data); }
  ...
  return <>
    <Modal open title="소싱 가격·재고 갱신" width={1180} onCancel={() => { if (!busy) onClose(); }} maskClosable={!busy} closable={!busy}
      ...
      <details className="pw-content-notices">
        ...
        <p>현재 iHerb·Vitabiotics·포트넘앤메이슨·코스트코 UK·Ocado 상품을 지원합니다. Ocado는 브라우저 접근이 필요합니다. 가격은 기존 소싱 배송비·묶음수량 규칙을 적용하고 판매가·최소마진 영향은 다음 검토에서 확인합니다. 소싱 실재고가 없으면 기존 값을 유지합니다. 판매용 설정 수량은 변경하지 않습니다. 함께 갱신할 항목은 한 번에 선택하세요. 일부를 적용하면 남은 항목은 다시 수집해야 합니다.</p>
      </details>
      ...
      {snapshots.length > 0 && <>
        <div className="pw-content-toolbar"><span>일괄 선택</span>
          <Button size="small" disabled={busy || collection.isError} onClick={() => selectField('PRICE')}>적용 가능한 가격</Button>
          <Button size="small" disabled={busy || collection.isError} onClick={() => selectField('STOCK')}>적용 가능한 재고 상태</Button>
    ...
    {review && <ProductSaveReview review={review} commit={productSourceApi.commit} onClose={() => setReview(null)} onSaved={saved} />}
  </>;
}
