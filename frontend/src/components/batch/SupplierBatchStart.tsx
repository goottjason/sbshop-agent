...
export function SupplierBatchStart({ onCreated, onCollapse }: { onCreated: (run: SupplierBatchRun) => void; onCollapse?: () => void }) {
  ...
  return <section className="sb-batch-start" aria-labelledby="supplier-batch-start-title">
    ...
    <p className="sb-batch-scope">판매금지·확인되지 않은 판매 재개는 보류하며, 마켓별 지원 조건과 실제 재조회 결과를 표시합니다.</p>
    ...
    {activeMarkets.includes('ELEVEN_STREET') && <p className="sb-batch-market-note" role="note">11번가는 현재 판매 중 상품의 수량 조정만 지원합니다. 가격·품절 전환은 보류됩니다.</p>}
