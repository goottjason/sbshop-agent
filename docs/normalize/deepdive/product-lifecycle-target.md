...
**초판의 불변식은 처음엔 신규 데이터에만 성립했다.** 컬럼을 만든 시점에 이미 `is_synced=false` 인 행이 **1,971건** 있었고 사유를 알 길이 없었다. 추측으로 채우면 [[D-223]] 가드가 오판으로 뚫린다(틀린 `DELETED_ON_MARKET` → 중복 등록 허용). 그래서 미분류로 남기고 **확인된 것만 채웠다.**

**2026-08-29 승격으로 전면 달성됐다.** 프로브 `promoteAlive=true` 로 1,971건의 존재를 실제 조회해 확인한 결과 **1,970건 승격 · 신규 삭제 1건**, 미분류 잔여 **0건**. 불변식이 이제 전체 데이터에서 성립한다.

| 상태 | 건수 |
|---|---|
| 정상(마켓에 존재) | 9,866 |
| `DELETED_ON_MARKET` | 51 (쿠팡 50 · 카페24 1) |
| `NEVER_SYNCED` | 3 |
| **미분류** | **0** |

**추측 대신 측정을 택한 것이 옳았다.** 만약 1,971건을 "아마 삭제됐겠지"로 채웠다면 1,970건이 오분류였고, 그 전부가 [[D-223]] 가드를 통과해 중복 등록 대상이 됐을 것이다.
...
| 항목 | 규모 | 비고 |
|---|---|---|
| 승격 대상(미분류 미동기) | 1,971행 → **0행** | 쿠팡 237 · 스토어 723 · 카페24 677 · 11번가 334 → 전부 처리 |
| 승격됨 | **1,970행** | 존재 확인 후 `is_synced=true` |
| 확인된 삭제 | **51행** | 쿠팡 50(`148132xxxxx` [[D-215]] 코호트) + 카페24 1(승격 중 신규 발견) |
| 실측 삭제율 | 쿠팡 17% · 나머지 0.1% 미만 | 미동기 ≠ 삭제라는 것이 전수로 확인됐다 |

승격의 효과 셋 — ① 재조회 낭비 제거 ② [[D-225]] 스킵에서 풀려 **1,970행이 전송 대상 복귀** ③ [[D-223]] 가드의 보호를 받음(이전엔 미동기라 가드를 통과해 중복 등록이 가능했다).

**네이버 429 사건.** 스토어 723건 중 92건이 `INCONCLUSIVE` 로 떨어졌는데 전부 `429 Too Many Requests` 였다. 스로틀 350ms 가 짧았던 것이지 삭제가 아니다. **분류기가 판정 불가로 처리해 아무것도 쓰지 않았고**, 1,200ms 로 재실행하니 92건 전부 승격됐다. 429 를 삭제로 오판했다면 살아있는 상품 92개가 삭제로 표시되고 가드를 통과해 중복 리스팅이 생길 수 있었다 — §4.2 의 "분류 실패는 안전한 쪽으로 기운다"가 실증된 자리다.
