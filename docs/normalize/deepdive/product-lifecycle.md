...
**4단계. 배지 시각 분리** (D-222) — **미착수**
1단계가 끝났으므로 이제 근거가 생겼다. `unsync_reason` 이 API 응답(`MarketRegistrationResponse.unsyncReason`)으로 이미 내려가므로, 프론트가 배지를 갈라 그리기만 하면 된다. **다음 순번.**

**5단계. 대조 결과 영속화** (D-227) / **게시 복구 배치** (D-226) — **미착수**
D-227 은 특히 값이 커졌다. 과거 데이터 **2,021건이 `unsync_reason` 미분류**로 남아 있는데(11.3 참조), 이걸 채울 수 있는 유일한 경로가 카탈로그 대조다.
