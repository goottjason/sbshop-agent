...
- 상태: **부분해결(7일 내 종료 종결)·전송실패 원인규명 완료·조치 미실행** 2026-09-01
...
### D-263: 유한 판매기간 428건을 2999-12-31 로 만들 경로가 없다 (2026-09-01)

- 심각도: 표준(90일마다 수동 작업 반복) | 위치: 마켓플러스 · ESM
- 배경: 무제한 = 종료일 `2999-12-31`. 현재 2,595건이 무제한이고 **428건(판매중 341 · 판매중지 32 · 판매종료 55)만 유한**하다. 이 428건은 [[D-259]] 조치로 2026-12월·2027-01월까지 밀어놨으나 **90일마다 다시 밀어야 한다**.
- 막힌 경로 (실측):
  - 마켓플러스 `판매관리 > 기간연장` 다이얼로그 선택지는 **15/30/60/90일뿐** — 무제한 없음.
  - 계정설정 `판매기간 자동연장 = 무제한` 은 **판매종료(만료) 상품 수정 시에만** 발동. 2026-09-01 대조 실험(옥션 90일 vs G마켓 무제한, 동일 상품 전송)에서 **판매중 상품은 양쪽 다 종료일 불변** 확인.
  - ESM 판매자센터 직접 변경: **오늘 확인 못 함.** `item.esmplus.com`·`www.esmplus.com` 이 크롬 확장의 스크립트 주입을 차단(3회 시도 모두 `Script injection timed out`).
- 남은 후보:
  1. 마켓플러스 `일괄전송 > 마켓별 일괄수정` 팝업에 판매기간 항목이 있는지 — 이 팝업은 MCP 탭 그룹 밖에서 열려 자동화로 못 연다. **사용자가 직접 열어 확인**하면 판정된다.
  2. ESM 상품 목록의 일괄 다이얼로그에 판매기간 변경이 있는지([[esm-listing-operations]] 의 쓰기 경로).
  3. 가설 검증: 판매종료 상품이 수정 전송에 **성공**하면 2999-12-31 을 받는가. 오늘 시험 불가 — 판매종료 55건이 전부 재고 0 이라 [[D-262]] 로 전송이 막혀 있었다. D-262 배포 후에도 재고 0 이면 여전히 못 살린다. 자연 발생(재고 있는 상품이 만료)을 기다리거나, 1건을 골라 카페24에서 임시로 판매함 처리해 관찰해야 한다.
- 임시 운영: **90일 주기 수동 `기간연장`**. 절차는 [[D-259]] 에 기록된 4배치(100/100/100/41) 그대로.
- 상태: **발견** 2026-09-01
