...
- 수정 방향(미착수): ① `IherbScraperClient` 가 ID 추출 실패 시 **예외를 던지거나 UNKNOWN 을 반환**하고, 호출부는 **재고 상태를 바꾸지 않는다**. ② `StockCrawlerRouter` 의 defaultCrawler 폴백을 **제거** — 크롤러 없는 소싱처는 명시적 실패. ③ 잘못 품절 처리된 655건의 재고 상태를 **되돌린다**(사람 확인 필요 — 진짜 품절도 섞여 있을 수 있다). ④ OCD·VTB·TES·COK 크롤러 신규 구현.
- 상태: 발견
...
- 수정 방향(미착수): [[D-239]] 를 먼저 고쳐야 한다 — 크롤러 없는 소싱처를 자동 크롤에 넣으면 655건이 **다시** 품절로 덮인다. 순서: D-239 폴백 제거 → 소싱처별 크롤러 확보 → 스케줄 활성화(소싱처 전체 대상).
- 상태: 발견
