...
- 심각도: 중 · 상태: 수정완료(검증 PASS, 실서버 확인 대기 — f7473e81·416feb99)
- 조치: 기본값을 `env -i` 별도 프로세스로 고정, prune 배선·헬스 본문·롤백 사후확인(헬스·nginx·frontend reload)·지문 기록 시점·frontend 단독 배포 시나리오 단언 추가. 테스트 121→168, 변형 26건 사망. 가짜 도커로 원리적으로 못 잡는 것(지문을 `.Config`만으로 계산하는 변형)은 실서버 실행이 유일한 근거로 남는다.
...
- 심각도: 낮음~중 · 상태: 수정완료(검증 PASS, 실서버 확인 대기 — f7473e81·416feb99)
...
- 심각도: 중 · 상태: 수정완료(검증 PASS, 실서버 확인 대기 — 416feb99)
...
- 수정: 앞서 api·frontend 를 교체했다면 실패 직전에 nginx 를 다시 읽히고 그 사실을 메시지에 밝힌다. 첫 서비스에서 실패하면 reload 하지 않는다. `replace_service` 는 `die` 대신 `return 4`, 호출부가 종료한다.
