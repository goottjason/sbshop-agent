...
- 심각도: 낮음 · 리스크: 경량 · 상태: **발견** 2026-09-05
...
- 심각도: 중 · 리스크: 표준 · 상태: **발견** 2026-09-05
- 82건이 `고시유형코드에 해당하는 고시 항목 개수가 일치하지 않습니다` 로 거부. GET 으로 받은
  기존 XML 을 그대로 되돌려도 거부되므로 **상품 데이터 자체가 낡은 것**이다. 스토어
  `consumptionDateText`([[barcode-market-push]])와 동종의 옛상품 함정.
- 수정 방향: PUT 전에 고시 블록을 현행 유형코드 규격으로 재구성하는 보정(스토어 보정과 같은 위치).

### D-299 — 11번가 전체 라운드트립이 안 바꾼 필드까지 재검사받게 한다 (금칙어 실증)

- 심각도: 중 · 리스크: 경량 · 상태: **발견** 2026-09-05
