...
- 수정 방향: 품절 + 원가 없음이면 가격 산정을 건너뛰고 `sourceGone` 과 같은 경로로 재고 0 을
  보낸다. 원가가 없는데 가격을 지어내지 않는다는 D-253 규칙은 그대로 지킨다.
- 결과 분류도 손봐야 한다: 이건 `FAILED` 가 아니라 "재고만 반영됨"에 가깝다. [[D-269]] 의
  `PARTIAL_FAILED` 와 같은 층의 판단이다.
...
- 수정 방향: `BLOCKED_BY_MARKET` 과 마켓 부재(`is_synced=false`)는 **전송 대상에서 빼고 `skipped`
  로 집계**한다. 다만 영구 제외는 위험하다 — 심사는 언젠가 끝난다. 마지막 오류 시각 기준으로
  일정 기간(예: 7일) 지나면 한 번 다시 두드려 상태를 재확인하는 창을 둔다.
  [[is-synced-means-present-on-market]] 의 프로브 규율과 같은 결로 맞춘다.
...
- [[D-269]] 로 만든 결과 표의 가독성을 이 한 마켓이 무너뜨린다. 화면 한 줄에 들어가야 할 사유가
  다섯 줄짜리 HTML 이 된다.
- 수정 방향: 마켓 오류 사유를 저장하기 전에 태그를 제거하고 공백을 접는다. 저장 단계에서 없애야
  화면·로그·재실행 판단 모두가 덕을 본다. 프론트에서만 지우면 `details` 는 계속 쓰레기를 안는다.
