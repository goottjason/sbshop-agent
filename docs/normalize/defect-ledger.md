...
### D-249: margin_rate 가 레거시 이관값에서 멈춰 있어 재가격 배치가 아이허브 1,639건을 8.6% 인하한다 (2026-08-30, D-248 후속 추적 중 발견)
...
- 해결 방향: `margin_rate` 를 실제 판매가에서 **역산해 백필**한다. 그래야 컬럼이 현실을 반영하고, 배치를 돌려도 각 상품이 현재 마진을 유지한다.
  - **주의 — 백필 시점이 중요하다.** 원가가 변했으면 역산값이 오염된다(원가 상승분이 마진 하락으로 보인다). 따라서 **가격 재크롤 직전, 같은 원가 기준으로** 백필해야 한다.
...
- 해결 방향: `createPriceInfo` 에서 마진율을 `null` 그대로 둔다. 판매가 초기 계산은 `cost * (1 + margin/100)` 이므로 null 이면 기준가 폴백 경로를 타게 하거나, 생성 시점에 소싱처 정책을 조회해 채운다. **후자가 낫다** — 생성 직후에도 올바른 가격이 나온다.
- 상태: **발견** 2026-08-30
