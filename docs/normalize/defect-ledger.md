...
`supportsMarketCancel` 에 쿠팡·스토어를 더해 `cancelOrderToMarketplace` 로 태웠다. 이 메서드는
이름과 달리 `getPort(marketType)` 범용 라우터라 각 어댑터로 간다.

**로컬 기록 여부를 마켓별로 갈랐다** — 원장이 경고한 "재조회 결과가 `NEW` 라 취소가 통째로
사라진다"를 피하기 위해서다.

| 마켓 | 마켓 전송 | 로컬 `CANCELED` 기록 | 이유 |
|---|---|---|---|
| G마켓·옥션 | O | **X** | 재조회가 진실을 싣는 것이 [[D-265]] 3단계에서 검증됨 |
| 쿠팡·스토어 | O | **O** | 재조회가 취소를 곧바로 싣는지 **미검증** |
| 11번가 | X | O | 판매자 취소 API 부재 |

쿠팡·스토어의 재조회 반영 여부는 **실주문에 취소를 걸어야만 확인할 수 있어** 검증하지 않았다.
그래서 로컬에도 기록해 사용자가 누른 결과가 사라지지 않게 했다. 마켓은 여전히 진실이며 다음
동기화가 교정한다. 라이브에서 쿠팡 취소 후 재조회가 즉시 `CANCELED` 를 싣는 것이 확인되면
G마켓과 같은 규율로 좁힐 수 있다.

회귀 불변 테스트("COUPANG 은 마켓 호출 안 함")를 뒤집었고, 11번가가 섞이지 않는 것을 새
회귀로 고정했다.
...
- [[D-181]](쿠팡이 HTTP 200 에 실패를 담아 보낸 것을 놓친 결함)과 같은 계열이다. **봉투가
  200 이라고 성공이 아니다.**
- **착수 중 발견 — `confirmOrders`(발주확인)에도 같은 결함이 있고 그쪽은 이미 라이브다.**
  `OrderService.confirmOrder` 가 부르는 경로다. 취소보다 이쪽이 급했다.
- 수정: `requireAllSucceeded(root, flagField, actionLabel)` 하나로 두 메서드를 묶었다. 거부된
  건이 있으면 어느 주문이 몇 건인지 담아 예외를 던진다.
- **남긴 것**: `detail` 이 아예 없는 응답은 던지지 않고 경고만 남긴다. 그런 응답 모양을 관측한
  적이 없어서, **라이브인 발주확인 경로를 미관측 가정에 걸 수 없다.** 관측되면 그때 좁힌다.
- 상태: **수정완료(검증대기)** 2026-09-03
...
### D-274: 30일 밖 송장 불일치는 아무도 손대지 않는다 (2026-09-02, D-273 검증 중 발견)

- 심각도: P2 | 위치: 확증층(`OrderReconciliationService`) · 정기 동기화 창
- 라이브 실측: 쿠팡 송장 불일치 4건이 전부 **61~63일 전**(2026-07-01~03) 주문이라 정기 동기화 30일 창 밖이다. [[D-265]] 4단계의 쿠팡 자동 재전송이 **검증할 대상조차 없었다.**
- 두 겹의 공백이다:
  1. **정기 동기화는 30일만 훑는다** — 그 밖의 `sb_shipment` 는 갱신되지 않는다. [[D-273]] 이전에도 확증층 목록 조회가 422/429 로 실패하고 있었으므로 실질적으로 늘 이랬다.
  2. **확증층 프로브는 `shipping_status` 만 본다** — 사용자 결정은 "상태·마켓송장에만 적용"이었는데 `market_tracking_no` 를 가져오지 않는다. `OrderProbeResult` 에 송장 필드가 없다.
- 결과: 120일 창으로 상태는 추적하지만 **송장 대조는 30일까지만** 살아 있다. 오래된 불일치는 화면에 빨갛게 남기만 하고 자동 조치가 닿지 않는다.
- 방향(택일 필요): ① 프로브가 마켓 송장도 실어오게 해 확증층이 `applyMarketTracking` + 불일치 정책까지 태운다 ② 오래된 불일치만 따로 훑는 저빈도 배치 ③ 30일 밖은 수동 처리로 명시하고 화면에 그렇게 표시한다
- ①이 설계 의도에 가장 가깝다 — 확증층은 이미 4마켓 단건 조회를 하고 있어 응답에 송장이 들어 있다.
- 수정(2026-09-03): 확증층이 프로브가 실어온 `marketTrackingNo` 를 `Shipment.applyMarketTracking` 으로 반영하고, 이어서 `TrackingMismatchResolver` 를 태운다. 목록 조회가 닿지 않는 오래된 주문도 이제 송장이 대조되고, 쿠팡이면 자동 재전송까지 간다.
- 프로브가 송장을 안 실어오면 기존 값을 지우지 않는다 — 부분 응답으로 대조 근거를 날리지 않기 위해서다(테스트로 고정).
- 상태: **수정완료** 2026-09-03

### D-275: 택배사 코드 3종이 미매핑으로 떨어진다 (2026-09-02, D-270 조사 중 발견)

- 심각도: P3 | 위치: `ShippingCarrier.resolve`
- 라이브 로그: `알 수 없는 택배사 코드: '0006' → 미매핑(null) 처리` 가 `0006`·`0012`·`0019` 세 코드로 반복된다(2026-09-02 하루에만 다수).
- 결과: 송장번호는 들어오는데 **택배사가 `null`** 이라 화면에 배송사가 안 뜨고, 송장 조회 링크도 만들 수 없다.
- 확인할 것: 세 코드가 어느 마켓에서 오는지(로그 스레드는 `SyncWorker`), 그리고 각 코드의 실제 택배사. 카페24 `shipping_company_code` 체계로 보인다.
- **실측(2026-09-03): 데이터 피해는 거의 없다.** 송장이 있는 271건 중 택배사가 `null` 인 것은 **1건**뿐이고, 그마저 송장이 `00000000` 인 더미다(마지막 유입 2026-08-07 — `isMeaningfulTracking` 가드가 생기기 전 잔재). 쿠팡 200·스토어 36·11번가 22·G마켓 11·옥션 2건 모두 택배사가 채워져 있다.
- 로그가 시끄러웠던 것에 비해 실제 영향이 작았다. `0006`·`0012`·`0019` 의 정체는 **아직 모른다** — 재배포로 로그가 유실됐고, 경고에 마켓·주문번호가 없어 출처를 짚을 수 없었다.
- 조치(2026-09-03): 경고에 **출처(마켓·주문번호)를 넣었다.** `ShippingCarrier.resolve/fromMarketCode` 에 context 파라미터를 더하고 호출부 4곳(쿠팡·스토어·카페24 매퍼·카페24 배송건 조회)이 넘기게 했다. 다음에 발생하면 어느 마켓 어느 주문인지 즉시 드러난다.
- 남은 것: 코드 정체가 밝혀지면 매핑 추가. 우선순위는 낮다.
- 상태: **관측 장치 배포, 매핑 보류** 2026-09-03
...
- **라이브 검증 완료(2026-09-03)**: 조회 건수 로그를 넣고 확인했다 — `쿠팡 반품요청 조회 완료: 2건 (2026-08-04~2026-09-03)` / `쿠팡 교환요청 조회 완료: 0건`. **교환 API 가 정상 호출된다.** 0건은 최근 30일에 쿠팡 교환이 실제로 없어서이고, 같은 경로의 반품이 2건 잡히는 것으로 살아 있음이 확인된다.
- (처음 검증이 막혔던 이유) 쿠팡 클라이언트는 11번가와 달리 조회 완료 로그가 없고 `detectExchanges` 도 반영 0건이면 조용해서, "안 불렀다"와 "불렀는데 0건이다"가 구분되지 않았다. 쿠팡 클라이언트는 11번가와 달리 조회 완료 로그를 찍지 않고, `detectExchanges` 도 반영 건수가 0 이면 조용하다. 최근 쿠팡 교환 자체가 없어 반영 대상도 없었다. 진단 가능성이 마켓마다 갈리면 검증이 매번 막힌다 — 조회 건수 로그를 넣어 11번가와 수준을 맞췄다.
- 상태: **수정완료** 2026-09-03
...
### D-279: 수집 전에 취소된 주문은 영영 들어오지 않는다 (2026-09-03, D-278 라이브 검증 중 발견)

- 심각도: P2 | 위치: 11번가 주문 수집 경로(4개 상태별 목록) · 다른 마켓도 같은 구조일 수 있음
- 라이브 관측: [[D-278]] 클레임 목록이 `20260807090911423`(11번가, 8/7 주문)의 **취소완료**를 정확히 알려줬는데, **그 주문이 우리 DB 에 아예 없다.** 같은 기간 8/8·8/9 주문은 있다.
  ```
  11번가 취소완료 목록: 1건 색인, 주문번호=[20260807090911423]
  클레임 목록 신호 1건이 어느 주문과도 짝지어지지 않았다: 신호=[20260807090911423]
  ```
- 원인: 11번가 주문 수집은 **상태별 목록 4종**(결제완료·배송준비중·배송중·배송완료)에 의존한다. 주문이 수집되기 전에 취소되면 그 4종 어디에도 없어 **처음부터 들어오지 않는다.** 어제 카페24에서 확인한 "클레임 주문이 목록에서 사라진다"와 같은 성질이다.
- [[D-278]] 자체는 정상 작동한다 — 감지·색인·매칭 시도 모두 제대로 하고, 짝이 없다는 사실을 경고로 남긴다. **문제는 기록할 주문이 없다는 것.**
- 영향: 취소 주문의 매출·정산이 장부에서 통째로 빠진다. 마켓에는 있고 우리에겐 없는 주문이 얼마나 되는지 모른다.
- 방향(택일 필요): ① 클레임 목록에 있는데 DB 에 없는 주문번호를 단건 조회(`claimservice/orderlistalladdr`)로 끌어와 생성 ② 취소 목록을 수집 소스에 추가 ③ 짝 없는 신호를 '확인 필요' 목록으로 보내 사람이 판단
- ①이 자연스럽다 — 클레임 목록이 주문번호를 주고, 단건 조회 경로가 이미 있다. 다만 "없는 주문을 만든다"는 [[D-160]] 계열의 판정 권한 문제를 건드리므로 신중해야 한다.
- **범위 확정(2026-09-03)**: 이 문제는 **별도 클레임 API 를 쓰는 11번가·쿠팡에만** 생긴다. 카페24·스마트스토어는 클레임이 *주문 목록*에 실려 오므로 목록에 있으면 주문도 함께 생성된다(`createMissing`) — 구조적으로 짝 없는 클레임이 나오지 않는다.
- 규모 측정 착수(2026-09-03): 두 마켓 모두에 "신호는 있는데 짝이 없다" 경고를 넣었다. 정기 동기화가 30분마다 30일 창을 훑으므로 하루면 창 안의 누락이 드러난다.
  - 우리 쪽 현황: 최근 30일 주문 49건(쿠팡 23·11번가 8·G마켓 8·스토어 8·옥션 2) 중 취소로 기록된 것은 1건(스마트스토어)뿐이다. **11번가는 취소 0건으로 기록돼 있는데 마켓은 같은 기간 취소완료 1건을 알려줬다 — 취소 실적의 100% 가 누락이다.**
**측정 재시도 결과 (2026-09-04)**

**측정 방식에 결함이 있었다.** 짝 없는 신호를 `log.warn` 으로만 남겼는데, `docker logs` 는
**컨테이너 재생성으로 날아간다.** 2026-09-03 에 배포를 여덟 번 하는 동안 누적분이 매번
사라졌다. 하루치 규모를 재려던 계획이 무산됐다.

건진 것: 같은 신호(`20260807090911423`)가 컨테이너 5분 생존 구간에도 다시 찍혔고, **그 주문은
하루가 지나도 DB 에 없다.** 매 동기화마다 경고가 반복되고 스스로 해소되지 않는다 —
**저절로 들어오지 않는다는 것이 확인됐다.**

최근 30일 취소 기록: 쿠팡 22건 중 0 · 11번가 8건 중 0 · G마켓 9건 중 0 · 스토어 8건 중 1.

**선행 작업**: 짝 없는 신호를 **활동로그(`sb_action_log`)에 남긴다.** 로그가 아니라 DB 라야
배포를 넘어 누적되고, 그래야 규모가 드러난다. 작고 즉시 가능하며, 이걸 해야 ①②③ 중 무엇을
할지 근거를 갖고 고를 수 있다.

**측정 재설계 완료 (2026-09-04)**

`ClaimOrphanRecorder` 가 마켓별로 **하루 한 행씩 `sb_action_log`** 에 남긴다
(`{MARKET}_CLAIM_ORPHAN`, `WARNING`). 30분마다 도는 동기화가 활동로그를 덮지 않도록 같은 날
이미 남겼으면 건너뛴다. **배포를 넘어 누적되므로 이제 규모를 잴 수 있다.**

함께 고친 것 — **11번가 판정 오류**. `orderCount == 0` 일 때만 경고해서 신호 3건 중 1건만
짝이 없으면 침묵했고, 신호가 0건이면 "0건이 짝지어지지 않았다"는 잡음을 냈다. 이제 우리 DB 에
없는 주문번호를 실제로 골라낸다. 쿠팡 쪽은 원래 옳게 계산하고 있었다.

**하루 뒤 확인할 것**:
```sql
SELECT action_type, message, created_at FROM sb_action_log
WHERE action_type LIKE '%_CLAIM_ORPHAN' ORDER BY created_at DESC;
```
누적된 주문번호 집합이 규모다. 그 크기를 보고 ①(단건 조회로 생성) ②(취소 목록을 수집 소스에
추가) ③(사람이 판단할 목록으로) 중 무엇을 할지 고른다.

**규모 판독 (2026-09-05)** — 하루치 활동로그 전수:

```
ELEVEN_STREET_CLAIM_ORPHAN | 1건: 20260807090911423 | 2026-09-04 00:50
```

**30일 창 내 짝 없는 클레임은 11번가 1건이 전부다. 쿠팡은 0건.** 매 동기화(30분 간격)가
같은 신호를 재관측하지만 하루 1행 규칙이 잘 눌렀다.

**판정: 자동 생성 파이프라인(방향 ①)은 과투자다.** 규모 1건에 "없는 주문을 만드는" 판정
권한 구조([[D-160]] 계열)를 세울 이유가 없다. 활동로그 경고가 곧 검토 목록(방향 ③)이며
이미 가동 중이다. 신호가 커지면 그때 ①을 다시 꺼낸다.

남은 1건(`20260807090911423`, 8/7 취소완료)은 수동 등재 대상 — 사용자 판단에 맡긴다.

- 상태: **측정 완료 · 경보 체제 유지(규모 1건 — 자동화 불요)** 2026-09-05
...
| ~~4~~ | ~~**[[D-270]]**~~ | **완료·라이브 검증 2026-09-02~03.** 1~4단계 + 4마켓 매퍼 + 클레임 필터 + [[D-277]]·[[D-278]] API 연결까지. 구매확정 24건·클레임 원본코드 수집 확인 | 중~높 | — |
| ~~5~~ | ~~**[[D-267]]**~~ | **완료 2026-09-02** — 원문 저장 + `last_sync_error_at` 신설 | 낮 | — |
| 6 | **[[D-275]]** | 택배사 코드 `0006`·`0012`·`0019` 미매핑 — 송장은 있는데 택배사가 `null` | 낮 | — |
| 7 | **[[D-274]]** | 30일 밖 송장 불일치가 방치된다 — 프로브가 `marketTrackingNo` 를 실어오게 해뒀으니 확증층에서 적용만 하면 된다 | 중 | [[D-270]] 3단계(완료) |
| 8 | **[[D-279]]** | 수집 전 취소 주문 누락 — **측정 장치 배포됨(2026-09-03). 하루 뒤 로그로 규모 판단 후 착수** | 중 | 측정 |
| 9 | **[[D-266]]** | 스마트스토어 옛 상품 `originProduct.statusType` 400 (66건). [[D-267]] 완료로 선행 풀림 — 배치 한 번 돌리면 사유가 쌓인다 | 중 | [[D-267]](완료) |
| 10 | **[[D-269]]** | 배치 결과·활동로그 프론트 UI/UX 전면 개편 — 부분 실패가 성공으로 보이는 문제 | 높(프론트 전면) | [[D-267]]·[[D-265]] 4단계 |
| 11 | **[[D-272]]** | 쿠팡·11번가·스토어 취소가 마켓에 전파되지 않는다 — 우리 장부에만 있는 취소 | 중 | 마켓별 취소 API 조사 |
| 12 | 잔여 | [[D-260]] 쿠팡 임시저장 232건 · [[D-261]] 쿠팡 품질 경고 · [[D-259]] 전송실패 543건 · [[D-240]] 정기 크롤 활성화 | — | — |
...
**선행 조건 재확인 (2026-09-04)**

**[[D-239]] 655건 복구는 완료됐다 — 독립 재측정으로 확인.**

| 소싱처 | 활성 | 품절 | 품절률 | 판정 |
|---|---|---|---|---|
| OCD | 271 | **0** | **0.0%** | 완전 회복 |
| VTB | 67 | 11 | 16.7% | 회복 |
| COK | 34 | 27 | 79.4% | 실크롤러로 잰 **진짜** 품절 |
| IHB·FTN | 2531 | 590 | — | 원래 영향 없던 소싱처 |

`StockCrawlerRouter` 의 폴백도 제거됐다 — 크롤러 없는 소싱처는 예외를 던지고 다른 크롤러로
대체하지 않는다.

**그러나 원장의 "지금 켜도 IHB·FTN 만 도니 해는 없다"는 더 이상 맞지 않는다.**
`ScraplingCrawlerConfig` 가 FTN·VTB·OCD·COK·TES 를 모두 등록해 `supportedVendors()` 가
**6개 전부**가 됐다(8/30 이후 변경).

**새로 걸린 선결 조건 둘** — 둘 다 켜기 전에 고쳐야 한다:
- [[D-290]] 대상 산출(`findByVendor`)이 **삭제된 291건을 포함한다.** 켜면 매일 반복된다.
- [[D-289]] 아이허브 파싱 실패가 아직 "품절"로 단정한다. [[D-283]] 배포로 그 오판이 이제
  마켓까지 곧장 나간다.

- 상태: **부분 수정 · 스케줄 활성화 대기(D-289·D-290 선행)** 2026-09-04
...
### D-248 재측정 (2026-09-03) — 실질 해소, TES 만 잠재 잔여

`MarketSalePriceResolver.resolveCouponRate/resolveMinMarginPrice` 는 이제
**상품별 → 소싱처별 → 전역** 순으로 찾는다(2026-09-01 소싱처별 가격정책 작업). 소싱처 정책 실측:

| 소싱처 | 마진율 | 쿠폰율 | 최소마진 |
|---|---|---|---|
| COK · FTN · OCD · VTB | 20~25 | **0** | 5000 |
| IHB | 4 | **0** | 0 |

전역(`sb_price_policy`)은 여전히 `couponRate=20` 이지만 **다섯 소싱처가 모두 자기 정책으로
0 을 덮으므로 전역값에 닿지 않는다.** 원장이 지목한 "COK 이 20% 할인 매입으로 계산된다"는
해소됐다.

**남은 것 — `TES` 98건은 소싱처 정책 행이 없어 전역 20% 로 떨어진다.** 다만 실측상
**마켓 등록 0건 · 원가 0** 이라 아직 아무 데도 팔리지 않는다. **잠재 결함이지 진행 중인
손실이 아니다.** TES 를 마켓에 올리기 전에 정책 행을 만들면 된다.

- 상태: **실질 해소 · TES 정책 행 추가 필요(발행 전)** 2026-09-03
...
### D-259 잔여(전송실패 543건) 재측정 — 2026-09-03

**결론: 오늘 수정들은 마켓플러스 전송실패를 늘리지 않았다. 측정으로 확인했다.**

**① 원장의 옛 진단은 이미 해소돼 있었다.** "환율 변동만으로 `changed` 가 매 배치 참이 된다"는
[[D-262]] 가 2026-09-01 에 고쳤다. 현재 코드는
`changed = statusChanged || (priceChanged && status != OUT_OF_STOCK)` 로, 품절 상품은 가격이
변해도 재전송하지 않는다. 그대로 살아 있다.

**② [[D-283]] 이 카페24 트래픽을 늘린다는 우려는 사실이 아니다 — 실측 0건.**
배치 `ca1de99f`(D-283 배포 후 실행)의 품절 경로 316건이 마켓별로 어디에 갔는지:
...
카페24는 한 건도 안 나갔다. 이 상품들은 우리 DB 에서 **이미 `OUT_OF_STOCK`** 이라
`soldOutChanged=false` 가 되고, 카페24의 "변경없음 스킵"이 그대로 걸린다. 즉 D-283 은
**마켓플러스로 내려가는 경로를 건드리지 않는다.** 543건과 무관하다.

**③ 우리가 원천적으로 볼 수 없는 것.** 카페24 API 는 `selling="F"` PUT 에 성공을 돌려주고,
그 뒤 마켓플러스가 G마켓/옥션으로 자동 전송하다 거부당하는 것은 **마켓플러스 화면에만 남는다.**
우리 DB 에는 성공으로 기록된다. **전송실패 543 카운터의 증감은 코드로 측정할 수 없고 화면을
봐야 한다.** 이것이 이 결함이 오래 방치된 구조적 이유다.

**조사 과정에서 바로잡은 것 두 가지** (서브에이전트 보고의 오류):
- "두 배치 모두 수정 전 코드로 돌았다"는 틀렸다. 실측 타임라인 —
  `f7c474a3` 09:16~10:23(D-283 경로 **0건**) → 배포 10:37:46 → `ca1de99f` 10:46~11:43
  (D-283 경로 **316건**). `ca1de99f` 는 D-283·284·285 를 반영한 코드로 돌았다.
  "소싱처 품절 → 재고 0 전송" 문자열 자체가 D-283 에서 생긴 것이라 이것이 결정적 증거다.
- "D-283 이 316건 규모의 신규 카페24 전송을 만든다"는 추측도 틀렸다. 실측 0건이다.

**남은 것**: 543 카운터의 현재값 확인은 라이브 크롬으로 카페24/마켓플러스 화면을 봐야 한다.
코드로는 더 잴 것이 없다. 판매 손실은 없다(재고 0 상품의 정상 귀결)는 기존 판단은 유효하다.
...
**확인된 사실 1 — 우리는 임시저장을 "성공"으로 알고 있다.** `sb_market_registration` 에는
쿠팡 승인상태(임시저장/심사중/판매중)를 담는 컬럼이 **아예 없다.** `is_synced`(마켓에 존재)와
우리 쪽 `status` 뿐이다. 그래서 임시저장 상품이 판매중 상품과 **완전히 동일하게**
`is_synced=true · unsync_reason=null · last_sync_error=null` 로 기록된다.

직접 증거: `market_detailed_info` 에 쿠팡 `statusName` 이 남아 있는 행은 전체 중 20건뿐인데
(대부분 등록 시점엔 `{}` 로 저장되고 별도 갱신이 있어야 채워진다) 그 분포가 이렇다 —
**임시저장 16 · 승인완료 2 · 승인반려 1 · 심사중 1.** 임시저장 16건 전부 `is_synced=true` 다.
**이것이 232건이 화면에서 안 보이는 이유다.**

**확인된 사실 2 — 승인요청 도구는 딱 한 번, 4건에만 쓰였고 전부 실패로 기록됐다.**
`sb_action_log` 의 `PRODUCT_APPROVAL_REQUEST` 전수(8행 = 4건 × 2회):

| 시각 | code | message |
|---|---|---|
| 08-27 05:49 (4건) | null | `411 Length Required` ([[D-215]] 로 당일 수정됨) |
| 08-27 06:13 (4건) | `ERROR` | `IN_REVIEW by seller gating` |

`CoupangMarketClient.requestApproval` 은 `code` 가 `SUCCESS`/`200` 이 아니면 전부 `failed` 로
떨어뜨린다. 그래서 두 번째 시도도 FAILED 로 남았다.

**검증하지 못한 것 — "실제로는 접수됐다"**. 2026-08-28 인계 문서에 "승인 요청 도구로
임시저장→심사중 전환은 실증됨"이라고 적혀 있으나, **우리 DB 로는 확인할 수 없다.** 그 4건의
`market_detailed_info` 는 오늘 배치의 가격·재고 응답으로 덮여 `statusName` 이 사라졌다.
**따라서 `IN_REVIEW by seller gating` 을 성공으로 재분류하는 수정은 지금 하면 안 된다** —
틀리면 진짜 실패를 성공으로 세게 된다. 마켓에서 그 4건의 현재 상태를 직접 확인하는 것이 선행이다.

**우리가 할 수 없는 것 — 쿠팡 seller gating.** 메시지의 `by seller gating` 이 가리키는 계정
차단은 판매자 대표 메일 확인·회신으로만 풀린다([[D-209]] 와 같은 뿌리). 이게 안 풀리면
아래 수정을 다 해도 232건은 승인완료로 넘어가지 않는다.

**조치 순서**
1. (사용자) 쿠팡 대표 메일·WING 문의에서 gating 해소 여부 확인 — **나머지 전부의 선행 조건**
2. (확인 후) 그 4건의 실제 마켓 상태를 보고 `IN_REVIEW` 재분류 여부 판정
3. (독립적으로 가능) **쿠팡 승인상태를 우리 DB 에 기록한다.** 임시저장을 성공과 구분하지 못하는
   한 232건은 계속 안 보인다. 이게 이 결함의 알맹이다.
4. (작음) `CoupangMarketClient.publish` 는 `code` 를 안 보고 `data` 유무만 본다 — 다른 메서드가
   쓰는 봉투 검사([[D-181]])와 일관되지 않다. 232건의 직접 원인은 아니나 같은 함정이다.

- 상태: **조사완료 · 착수대기(gating 확인 선행)** 2026-09-03
...
**조사 결과 — 브랜드 181건의 원인 확정 (2026-09-03)**

**쿠팡 화면 실측은 못 했다.** `wing.coupang.com` 은 브라우저 도구의 안전 제한으로 차단된다.
대신 우리 데이터를 직접 쟀고, 그것만으로 원인이 드러났다.

**브랜드가 영문 원문의 첫 단어로 잘려 있다. 3,195건 중 3,183건(99.6%).**

```sql
count(*) FILTER (WHERE split_part(original_name,' ',1) = brand)  →  3183 / 3195
```

실제 값과 있어야 할 값:

| 저장된 brand | 상품수 | 실제 브랜드 |
|---|---|---|
| `Nature's` | 97 | Nature's Way |
| `Doctor's` | 76 | Doctor's Best |
| `California` | 71 | California Gold Nutrition |
| `Garden` | 60 | Garden of Life |
| `Sports` | 56 | Sports Research |
| `Life` | 52 | Life Extension |
| `Nordic` | 38 | Nordic Naturals |
| `Optimum` | 33 | Optimum Nutrition |

부수 실측: **숫자로 시작 45건**(`28 Piece Ginger…` → `28`), **세 글자 이하 409건**(`A`·`Dr.`·`NOW`),
**마침표로 끝남 34건**(`Dr.`). 그리고 `NOW` 93건과 `Now` 56건이 **대소문자만 다른 같은 브랜드**로
갈려 있다.

**고칠 수 있다 — 진짜 브랜드는 이미 크롤러가 읽고 있다.**
`IherbScraperClient` 는 아이허브 응답의 `brandName` 을 파싱해(`:172`) `ScrapedProductDto.brand`
로 싣는다(`:311`, `toScrapedDto`). 즉 **소싱처가 정확한 브랜드를 주고 있는데 기존 상품에
반영하는 경로가 없다.** 오늘 돌린 가격·재고 배치는 `ProductUpdateCommand` 에 brand 를 넣지
않으므로 2,146건을 크롤하고도 브랜드는 그대로였다.

**수정 방향**: [[D-175]] 바코드 백필과 같은 모양의 **브랜드 백필**을 만든다 —
`crawlProductInfoAsDto` 로 `brandName` 을 읽어 `sb_product.brand` 를 갱신하고, 쿠팡에 재전송한다.
바코드 백필이 이미 그 뼈대(배치·스로틀·`ProcessStatus` 기록)를 갖고 있으니 재사용한다.

**주의 두 가지**
1. **쿠팡 브랜드 마스터와의 일치는 별개 문제다.** 아이허브 표기(`Nature's Way`)가 쿠팡이 받는
   표기와 같다는 보장이 없다. 소량으로 먼저 전송해 쿠팡이 경고를 거두는지 확인한 뒤 확대할 것.
2. **[[D-260]] 의 seller gating 이 안 풀리면 쿠팡 반영이 막힌다.** 브랜드 값 교정(우리 DB)은
   지금 가능하고, 마켓 반영은 gating 해소가 선행이다.
...
### D-261 브랜드 언어 판정 (2026-09-04, 사용자 지적으로 착수)

사용자가 "쿠팡은 한글로 브랜드명을 표기하는 것으로 안다"고 지적했다. **맞았고, 우리 구현이
정반대였다.** 백필을 돌리기 전에 잡았다.

**실측 — 우리가 보낸 값과 쿠팡이 저장한 값의 짝 (20건 전수)**

| 우리가 보낸 값 | 쿠팡 저장값 |
|---|---|
| `21st` · `NaturesPlus` · `Nature's` · `Nordic` · `Organic` · `Julian` · `Pure` | 21세기센트리 · 네이처스플러스 · 네이쳐스웨이 · 노르딕내추럴스 · 오가닉인디아 · 줄리안베이커리 · 퓨어인디안푸즈 |
| `Enzymedica` · `Comvita` | 엔자이메디카 · 콤비타 |
| `Bluebonnet` · `Thorne` · `Vitabiotics`(2) · `Dastony` · `NuTrail` · `Paradise` · `Now` · `Real` · `Four` | **자체브랜드** (10건) |
| `Bahlsen` | **상품상세 참조** (1건) |

`자체브랜드`·`상품상세 참조` 는 **우리 코드에 없는 문자열이다**(grep 0건) — 쿠팡이 채운
폴백이다. 즉 쿠팡은 받은 값을 **자기 마스터의 한글 정식명으로 정규화**하고, 못 찾으면
`자체브랜드` 로 떨어뜨린다.

**서브에이전트 해석 하나를 정정했다.** "영문이라 실패했다"가 아니다 — `Enzymedica`·`Comvita`
는 **완전한 영문인데 성공**했고 `Bluebonnet`·`Thorne`·`Vitabiotics` 는 **완전한 영문인데 실패**했다.
실패 원인은 언어가 아니라 **그 브랜드가 쿠팡 마스터에 있느냐**다.

**그럼에도 한글을 택한 근거 — 공백이다.**

- 매칭에 성공한 9건의 저장값은 **전부 공백이 없다.**
- 백필 대상 2,893건이 **전부** `original_name` 에 공백을 갖는다. 즉 영문 브랜드를 온전히 넣으면
  `Nature's Way`·`Garden of Life`·`California Gold Nutrition` 처럼 **거의 항상 공백이 들어간다.**
- 한글 표기는 구조적으로 공백이 없다(`네이처스웨이`·`가든오브라이프`).
- 쿠팡 공식 문서(`docs/external-api/coupang/쿠팡상품수정(승인필요).pdf`)에 "브랜드명은 한글/영어
  표준이름 입력, 띄어쓰기 및 특수문자 없이 입력" 이라는 규정이 있다고 조사가 보고했다.
  **이 인용은 내가 직접 확인하지 못했다**(PDF 렌더러 부재). 다만 위 두 실측이 같은 방향을
  가리킨다.

**수정**: `extractBrand` 가 괄호 **안**의 한글을 취하고 공백을 지운다. 괄호가 없으면 원문을
그대로 쓴다(한글 표기를 안 주는 소싱처 대비).

**남은 위험 — 정직하게 적는다**
1. **표기 불일치.** 쿠팡은 `네이쳐스웨이`, 아이허브는 `네이처스웨이`(쳐/처). 한글로 바꿔도
   마스터와 안 맞으면 여전히 `자체브랜드` 로 떨어진다. 쿠팡 브랜드 조회 API 문서를 확보하지
   못해 사전 검증할 수단이 없다.
2. **다른 세 마켓은 증거가 없다.** 11번가·스토어·카페24 는 브랜드를 `market_detailed_info` 에
   왕복 저장하지 않아 그쪽이 무엇을 원하는지 알 수 없다. 코드상 네 마켓이 `product.brand` 하나를
   공유한다.
3. **브랜드 변경은 쿠팡에서 승인필요 수정이다.** 문서 제목부터 "상품수정(승인필요)"이다.
   [[coupang-republish-attributes]] 의 "PUT 성공 = 심사중 전환" 경고와 맞물린다.
   **[[D-260]] seller gating 이 안 풀린 상태에서 2,893건을 밀면 대량이 심사중에 갇힐 수 있다.**
   그래서 이 작업은 **우리 DB 교정까지만** 하고 마켓 전송은 하지 않는다.

- 상태: **백필 코드 완료(한글) · 대량 실행 대기** 2026-09-04

**쿠팡 공식 브랜드 검색 API 연동 완료 (2026-09-04, TDD)**

사용자가 쿠팡 브랜드 검색 API 문서(`docs/external-api/coupang/brand-api.md`)를 확보하면서 위
"한글 추정" 방식을 쿠팡이 실제로 인정하는 표기로 교체했다. 아이허브 `brandKo`에서 영문·한글 후보를
둘 다 뽑아 `POST .../marketplace/brands/search`로 둘 다 조회하고(정규화 후 정확일치만 매칭,
`totalCount>0`은 매칭 아님, `code` 봉투 검사 필수), 둘 다 매칭되면 한글 쪽을 쓴다. 쿠팡에 없으면
크롤 값을 쓰되 `ProcessStatus` 메시지에 "쿠팡 미등록"으로 남긴다(빈 값으로 덮지 않음, [[D-253]]).

신규: `CoupangBrandLookupPort`(core 포트) · `CoupangBrandLookupService`(infrastructure, 브랜드
단위 캐시 — 매칭 성공·실패 모두 캐시해 같은 브랜드 반복 조회 시 API 1회만 호출, 테스트로 증명) ·
`GET /internal/coupang/brand?name=...`(단건 확인 엔드포인트). `ProductBrandBackfillService`는
`extractBrand` 단일 메서드를 후보 분리(`parseBrandKo`)와 쿠팡 우선 해석(`resolveOfficialBrand`)
으로 재구성했다. 이 프로젝트에 `@EnableCaching`/`CacheManager`가 없어 `CoupangMetaService`의
기존 `@Cacheable`이 무효 상태임을 확인했고(별도 결함 후보로만 남김, 이번 범위 밖), 그래서 캐시는
Spring 어노테이션이 아니라 서비스 내부 `ConcurrentHashMap`으로 직접 구현했다.

TDD로 3계층(인프라 9케이스·코어 14케이스[기존 11 + 신규 3]·API 컨트롤러 2케이스) 전부 Red 확인
후 Green. 넓은 회귀(`:core:test :infrastructure:test :api:test`, product 패키지 전체 + coupang
클라이언트 전체)도 실패 0건. 대량 실행·DB 쓰기·마켓 전송은 하지 않았다 — 사용자가 단건 확인
엔드포인트로 검색 API가 실제로 동작하는지 먼저 본 뒤 소량부터 판단한다. 산출물:
`_workspace/fixes/D-261_coupang_brand_lookup_fix.md`.
...
- **원인 확정(2026-09-03)**: `normalizeReadOnlyStatusType` 이라는 보정 함수가 이미 있었는데 **가격·재고 경로에서 불리지 않았다.** 바코드·고시정보 경로(248·279줄)에서만 쓰였다. 게다가 그 함수는 `OUTOFSTOCK` 만 치환하고 **빈 값·누락은 손대지 않았다** — 옛 상품이 겪는 것이 바로 그 경우다.
- 품절 분기가 결정타였다. `soldOut` 이면 `stockQuantity=0` 만 넣고 `statusType` 을 아예 건드리지 않아, **실패 66건 중 65건이 품절 상품**이었다(FTN 9·OCD 19·VTB 1·IHB 36).
- 수정: 보정 함수가 빈 값·누락도 `SALE` 로 채우게 하고, 품절 여부와 무관하게 가격·재고 경로에서 항상 부르게 했다. `SUSPENSION` 같은 실제 값은 건드리지 않는다 — 우리가 판매 상태를 바꾸지 않는다.
- 상태: **수정완료(라이브 검증 대기)** 2026-09-03
- 착수 시 확인할 것
- 상태: **발견** 2026-09-02
...
- 수정(2026-09-02): 두 호출부(`ProductMarketSyncService:121`·`ProductBarcodeSyncUseCase:102`)를 2-arg 로 교체해 원문을 남긴다. 재실패 시각 문제는 **`last_sync_error_at` 컬럼 신설**로 푼다 — 같은 오류가 반복되면 다른 필드가 안 바뀌어 JPA 가 UPDATE 를 내지 않으므로, 시각을 직접 들고 있어야 갱신 흔적이 남는다. `markSynced()` 는 이 시각도 함께 지운다.
- DDL: `ALTER TABLE sb_market_registration ADD COLUMN IF NOT EXISTS last_sync_error_at TIMESTAMP;` — **운영 적용 완료 2026-09-02**
- 상태: **수정완료** 2026-09-02
...
- **정정(2026-09-02): 이 수정은 방향이 반대였다.** 아이템 `E*` 를 종결로 확정하게 만들었는데, 실측 결과 아이템 코드는 교환 *이력*을 붙들고 있고 주문 레벨 값이 마켓 화면과 일치했다. 교환은 종착역이 아니라 경유지다. [[D-270]] 3단계에서 이 분기를 걷어내고 클레임을 독립 축으로 뺀 뒤에야 제대로 닫혔다 — 4477134670 이 `SHIPPED` + `EXCHANGE/REQUESTED(E00)` 로 정상화됐다(2026-09-02 08:27 라이브 확인).
- 상태: **[[D-270]] 으로 대체·해소** 2026-09-02
...
**직접 겪은 관찰 (2026-09-03, [[D-266]] 배치 실행 중)**

착수 전 근거를 실사용으로 모았다. 아래는 추정이 아니라 오늘 화면에서 겪은 것이다.

1. **완료 표시가 거짓말을 한다.** 진행현황에 `완료 (성공 3 / 실패 0)` 으로 남아 있는 배치가, 실제로는 스마트스토어 1건이 400 으로 실패한 어제 그 배치였다. 화면만 보면 성공으로 읽힌다.
2. **실행 대화상자의 버튼에 라벨이 없다.** 상품관리 → "선택 가격/재고 업데이트" 대화상자의 확인·취소가 접근성 트리에 `button (unnamed)` 두 개로만 나오고 화면에도 흰 박스로 보인다. **어느 쪽이 실행인지 알 수 없어 세 번 헛클릭했다.**
3. **결과 메시지가 마켓별로 갈리지 않는다.** `마켓반영 성공1/스킵1/실패1 ([SMART_STORE])` 처럼 실패 마켓이 붙을 때가 있고, `성공0/스킵0/실패2` 처럼 **어느 마켓이 실패했는지 아예 없을 때가 있다.** 후자는 로그를 봐야만 알 수 있다.
4. **실패 사유가 화면에 없다.** 오늘 OCD·VTB 20건이 전부 실패했는데 화면으로는 원인을 알 수 없었다. 서버 로그를 읽고서야 `404 존재하지 않는 상품입니다`(마켓에서 이미 삭제됨)임이 드러났다 — [[D-266]] 의 `statusType` 400 과 **완전히 다른 원인**인데 화면에서는 구분되지 않는다.
5. **상품 선택 경로가 실용적이지 않다.** 상품관리에서는 조건으로 좁힐 수 없어 SB코드로 한 건씩 검색해야 한다. 배치 페이지의 "상품ID 지정" 탭이 훨씬 나은데, **상품 ID 를 알아야 해서 DB 를 봐야 한다.** "실패한 것만 다시" 같은 재실행 경로가 없다.

**A단계 완료 — 집계 정의 교정 (2026-09-03 라이브 검증)**

`ProcessStatusType.PARTIAL_FAILED` 신설. 마켓 전송이 하나라도 거부되면 `markSuccess` 대신
`markPartialFailed` 로 기록한다. 대상은 마켓에 실제로 보내는 세 경로(크롤 정상·소스링크
사망·수동 배치). 마켓 전송이 없는 전체필드 수정은 그대로 `SUCCESS`.

같은 상품(3020)을 고치기 전후로 돌린 결과가 이것이다:

| 배치 | step | status | message |
|---|---|---|---|
| afc4a348 (전) | UPDATE_PRODUCT_SAVE | **SUCCESS** | … 실패1 `([SMART_STORE])` |
| 78d2274b (후) | UPDATE_PRODUCT_PUBLISH | **PARTIAL_FAILED** | … 실패1 `(SMART_STORE: 400 … "판매가 항목…)` |

화면도 `완료 (성공 0 / 부분실패 1 / 실패 0)` 로 바뀌었고, 활동로그는
`배치 완료(부분실패 1건)` 을 `FAILED` 로 남겼다. `sb_process_status` 에는 CHECK 제약이
없어 [[D-276]] 같은 DDL 사고는 없었다(배포 후 쓰기 성공으로 확인).

메시지에 실패 마켓 **이름만** 넣던 것을 **사유 원문까지** 넣도록 바꿨다(160자 절단).
B단계 결과 표가 로그 없이 원인을 보여줄 수 있어야 하기 때문이다.

남은 작은 것: 부분실패도 활동로그에서는 `FAILED` 로 뭉뚱그려진다. `BatchCompletedEvent`
가 boolean 만 나르기 때문. E단계에서 `WARNING` 을 들일지 함께 판단한다.

**B·C·D단계 완료 (2026-09-03 라이브 검증)**

**B — 결과를 행으로.** 마켓별 결과를 `details` 컬럼에 JSON 으로 남기고, `BatchResultTable`
컴포넌트가 그것을 읽는다. **메시지 문자열을 프론트에서 되파싱하지 않는다** — 방금 서버가
직렬화한 문자열을 다시 뜯는 방식은 사유에 따옴표와 중괄호가 섞이는 순간 깨진다(네이버 400
본문이 정확히 그렇다). `details` 는 이미 있던 컬럼이고 이 작업 유형에서는 비어 있었다.

실측 결과 화면(배치 `c2bd04fe`, 두 종류의 실패를 일부러 한 배치에 넣었다):

| 상품 | 결과 | 원인 | 내용 |
|---|---|---|---|
| 1916 | 실패 | 소싱처 | 매입 원가가 없다 — 판매가를 만들 수 없다 |
| 3020 | 부분실패 | 마켓 | N스토어 — 판매가 항목은 10원 단위로 입력해 주세요<br>전송됨: 11번가, 쿠팡 · 변경없음: 카페24 |

`원인` 배지를 **소싱처 / 마켓** 두 갈래로 나눈 것이 핵심이다. 전자는 마켓에 요청조차 못 간
것이고([[D-281]]) 후자는 마켓이 거부한 것이라 할 일이 전혀 다르다. 기본은 문제 건만 펼치고
성공은 접는다. 마켓 사유는 본문에서 사람이 읽을 문장을 뽑아 앞세우고 원문은 접어 둔다.

**C — 문제 건만 재실행.** 실행 조건은 배치 시작 시 저장해 두어 새로고침해도 남는다.
라이브에서 `문제 2건만 다시 실행` → 새 배치 `32914ca3` 로 두 건만 재실행 확인.

**D — 버튼이 안 보이던 진짜 이유.** 라벨이 빠진 게 아니었다. `--product-primary` 는
`.product-theme` 스코프 안에서만 정의되는데 Ant `Modal` 은 `document.body` 포털로 나간다.
그래서 `okButtonProps` 의 `background: var(--product-primary)` 가 아무것으로도 풀리지 않고
primary 버튼의 **흰 글자만 남아 빈 흰 박스**가 됐다. `rootClassName="product-theme"` 로
...
### D-280 — 네이버는 판매가를 10원 단위로만 받는다 (마켓별 가격 산정이 1원 단위를 만든다)

- 심각도: 중 · 리스크: 경량 · 상태: **수정완료(검증대기)** 2026-09-03
- 증상: 상품 3020 스토어 전송이 `400 BAD_REQUEST — originProduct.salePrice / "판매가 항목은 10원 단위로 입력해 주세요."` 로 거부됐다. 보낸 값은 `salePrice=20105`.
- 원인: [[D-094]] 마켓별 실수수료 산정이 마켓마다 다른 값을 만든다(같은 상품이 쿠팡 20500 · 11번가 22600 · 스토어 20105). 쿠팡·11번가는 1원 단위를 받지만 **네이버만 10원 단위를 강제한다.**
- 수정: 어댑터가 아니라 `MarketSalePriceResolver`(마켓별 판매가 산정 단일 관문)에서 `SMART_STORE`일 때만 10원 단위로 올림(`resolve`/`resolveForProduct` 두 진입점 모두, 원가·마진 미보유 폴백 경로 포함). 내림은 마진을 깎으므로 올림. 다른 마켓은 1원도 안 바뀜. 산정식 자체는 건드리지 않음 — 이건 마켓 제약이지 가격 정책이 아니다.
- 커밋 전 상태: `backend/core/src/main/java/com/sbshop/agent/core/application/product/MarketSalePriceResolver.java` + 신규 테스트 `SmartstorePriceUnitTest.java`(5케이스). 상세는 `_workspace/fixes/D-280_fix.md`.
- 발견 경위: [[D-266]] 검증 배치(`afc4a348`) 중. D-266 의 `statusType` 은 고쳐졌고(로그에 `OUTOFSTOCK → SALE 치환` 후 진행) 그 다음 단계에서 걸린 별개 원인이다.
...
- **원래 증상은 [[D-283]] 으로 해소됐다 (2026-09-03 확인).** 지목했던 4건(1916·2069·2096·2980)이
  모두 `정상` 이 됐다 — D-283 이 이 상품들을 마켓 전송까지 데려갔고, 쓰기에 성공하자
  `markSynced()` 가 옛 오류를 지웠다. 이제 크롤 문제는 상품에 기록되고([[D-255]]) 마켓 오류는
  실제 마켓 응답에서만 나온다. **"크롤 실패가 마켓 오류로 보인다"는 구조가 사라졌다.**
- **남은 알맹이 두 가지**
  1. **나이를 알 수 없는 오류 87건.** `last_sync_error_at` 이 빈 행들 — [[D-267]] 이전에 쓰였다.
     화면은 "마켓이 데이터를 거부했습니다"라고 말하지만 오늘 일인지 7월 일인지 알 수 없다.
     시각이 없으면 "이전 오류"로 구분해 보여줘야 한다.
  2. **스킵된 마켓은 옛 오류가 스스로 지워질 기회가 없다.** 이 중 카페24 19건은 표시 문제가
     아니라 실제로 멈춰 있었다 → [[D-286]] 으로 분리 등재.
...
- 심각도: **높음** · 리스크: 표준 · 상태: **발견** 2026-09-03
- 발견 경위: [[D-281]] 잔여 범위를 재다가 드러났다. 카페24 등록 **19건**이 `TRANSIENT_ERROR` 를
  단 채 `is_synced=true` 로 남아 있었다.
- 실측(오늘 배치 `ca1de99f` 기준):

  | 상품 | 마지막 성공 | 오늘 처리 |
  |---|---|---|
  | 1665 · 1671 · 1672 · 1680 · 1681 · 1696 … | **2026-08-24** | **스킵** |

  마지막 성공이 08-24 이고, 그 뒤 쓰기가 실패했으며, **그 이후 모든 배치에서 스킵됐다.**
  10일째 재시도가 한 번도 없었다.
- 원인: `ProductMarketSyncService.syncInternal` 의 카페24 분기가
  `!changed && Boolean.TRUE.equals(reg.getIsSynced())` 만 본다. **`lastSyncError` 를 읽지 않는다.**
  지난 쓰기가 실패했으면 "값이 안 바뀌었다"는 "보낼 게 없다"가 아니다 — 마켓은 지난 변경을
  아직 못 받았다.
- [[D-284]] 와 정확히 같은 병의 반대 방향이다. D-284 는 **막힌 걸 계속 두드렸고**, 이건
  **보내야 할 걸 계속 건너뛴다.** 둘 다 등록 상태 필드를 아무도 안 읽어서 생긴다.
- 수정: 카페24 스킵 조건에 `reg.getLastSyncError() == null` 을 더했다. 오류가 남아 있으면 값이
  안 바뀌었어도 다시 보낸다. 성공하면 `markSynced()` 가 오류를 지우고 다시 조용한 스킵으로
  돌아간다. **막힌 상태([[D-284]])는 이 규칙보다 우선한다** — 재시도로 안 풀리는 것을 다시
  두드리게 만들면 안 된다. 회귀로 고정했다.
- 상태: **수정완료(검증대기)** 2026-09-03
- 이 19건은 08-24 이후의 가격·재고 변경이 카페24(=G마켓·옥션 경유)에 반영되지 않았다는 뜻이다.
  실제 판매가가 우리 장부와 다를 수 있다.

### D-282 — 주문 유입 경보 임계가 마켓의 실제 주문 속도와 무관한 상수다

- 심각도: 중 · 리스크: 경량 · 상태: **수정완료(검증대기)** 2026-09-03
- 증상: `SMART_STORE_SYNC_STALE — 신규 주문이 2일째 0건이다 (임계 2일)` 경보. 그러나 수집은 정상이었다.
- **수집이 정상이라는 근거 3가지**
  - `sb_market_sync_status` 의 SMART_STORE 가 `COMPLETED`, `error_message` 없음, `last_sync_at` 은 당일 07:46 KST.
  - 그 회차에 **14건이 실제로 흘렀다**(`processed=14`). 네이버 주문 조회는
    `/pay-order/seller/product-orders/last-changed-statuses` — **변경분 피드**다. 14건이 통과했다는
    것은 파이프가 살아 있다는 뜻이고, 그중 신규가 0건이었을 뿐이다.
  - 신규 주문이 실제로 있었다면 변경분 피드에 잡혀 `신규`로 셌을 것이다.
- 진짜 원인: `SyncFreshnessPolicy` 의 임계가 마켓별 **하드코딩 상수**(스토어·쿠팡 2일, 11번가 7일,
  G마켓 10일)인데 마켓의 실제 주문 속도를 반영하지 않는다.
- 실측: 스마트스토어 최근 15건의 주문 간격은 `0.0 0.0 0.2 0.6 0.7 0.9 1.0 1.6 2.0 2.3 5.2 7.4 7.7 12.2 14.0`(일).
  **15개 중 5개가 2일을 넘는다.** 즉 정상 간격의 3분의 1에서 경보가 울린다. 월 7~10건(≈0.3건/일)
  마켓에 2일 임계는 조용한 날마다 울리는 설정이다.
- **정정(2026-09-03)**: 처음에 "쿠팡도 7월 131 → 8월 17 → 9월 8건으로 줄어 곧 오작동한다"고 적었으나
  틀렸다. 9월 8건은 **3일치**라 오히려 2.7건/일로 활발하다. 쿠팡은 90일 213건(평균 간격 0.4일)이라
  새 공식으로도 임계가 하한 2일에 걸린다 — 그 물량에서는 2일 공백이 실제로 이상 신호라 옳다.
  이 결함은 **저물량 마켓에서만** 발생한다.
- 왜 고쳐야 하나: 정상 상태에 울리는 경보는 무시하도록 사람을 훈련시킨다. 진짜 단절이 왔을 때
  ([[esm-order-collection-outage]] 같은) 묻힌다. 경보가 없는 것보다 나쁘다.
- 수정: 임계를 상수가 아니라 **그 마켓 자신의 평균 주문 간격의 배수**로 잡는다.
  `임계 = clamp(3 × (90일 / 90일내 주문수), 2일, 30일)`, 주문수 0이면 상한 30일(0으로 나누지 않는다).
  `SyncFreshnessPolicy` 는 주문수를 인자로 받는 순수 함수가 되고, `ActionLogSyncListener` 가
  건수를 조회해 넘긴다. **G마켓 동기화 키는 GMARKET·AUCTION 둘 다를 뜻하므로 건수도 합산한다** —
  안 그러면 임계가 실제의 두 배가 된다.
- 실측 적용 결과(2026-09-03 운영 DB 90일):

  | 마켓 | 90일 주문 | 평균 간격 | 새 임계 | 옛 임계 |
  |---|---|---|---|---|
  | 쿠팡 | 213 | 0.4일 | **2.0일** | 2일 |
  | N스토어 | 26 | 3.5일 | **10.4일** | 2일 |
  | 11번가 | 22 | 4.1일 | **12.3일** | 7일 |
  | G마켓+옥션 | 22 | 4.1일 | **12.3일** | 10일 |

  오늘 오탐을 낸 스토어는 2일 → 10.4일이 되어 2.6일 공백에 울리지 않는다. 물량이 많은 쿠팡은
  하한에 걸려 그대로 2일 — 필요한 곳의 민감도는 유지된다.
- 상태: **수정완료(검증대기)** 2026-09-03
- 관련: [[state-guard-case-by-case]] 와 같은 결의 문제 — 일괄 상수 대신 대상별로 정의해야 한다.

### D-283 — 원가를 못 읽으면 재고도 마켓에 안 보낸다 (품절 상품이 마켓에서 계속 팔린다)

- 심각도: **높음** · 리스크: 표준 · 상태: **발견** 2026-09-03
- 발견 경위: 아이허브 전체 배치(`f7c474a3`, 2146건). 실패 283건이 **전부** 이 한 가지 원인이었다.
- 사실: 크롤은 성공했다. 로그에 `아이허브 크롤링 결과 - status: OUT_OF_STOCK, costPrice: null` 이
  287회 찍혔다. 아이허브는 **품절이면 가격을 안 보여준다.** 크롤 실패가 아니라 정상 응답이다.
- 그런데 `LandedCostCalculator.buyPricePerUnit` 이 원가 null 에 예외를 던지고([[D-253]] 규칙,
  이 자체는 옳다), `BatchPriceStockService.crawlAndUpdatePriceStock` 의 catch 로 빠진다.
  **그 지점이 `product.updateStockStatus()` 와 `syncPriceStockPerMarket()` 앞이다.**
  결과적으로 재고 0 이 어느 마켓에도 나가지 않는다.
- 증거: 실패한 293건의 `sb_product.updated_at` 은 오늘(배치가 크롤실패를 기록하며 저장),
  그런데 `sb_market_registration.last_synced_at` 은 **08-31 에 멈춰 있다.** 3일째 마켓에
  아무것도 안 갔다.
- **바로 옆에 정답이 이미 있다**: 같은 메서드의 `result.sourceGone()` 분기는 원가를 모르는 채로
  `PricingInputs(null, ...)` 과 `OUT_OF_STOCK` 을 넘겨 **가격은 건드리지 않고 재고만 0 으로 보낸다.**
  소스가 죽은 경우와 품절인 경우는 "가격을 모른다"는 점에서 같은데 처리가 다르다.
- 수정: 품절 + 원가 없음이면 가격 산정을 건너뛰고 `sourceGone` 과 같은 경로로 재고 0 을 보낸다.
  크롤 실패로 기록하지 않고(실패한 게 아니다) 결과도 성공으로 집계한다. 원가가 없는데 가격을
  지어내지 않는다는 D-253 규칙은 그대로 지킨다.
- **재고가 있는데 원가가 없는 경우는 그대로 실패로 남겼다.** 품절이라 가격이 없는 건 아이허브의
  정상 동작이지만, 판매 중인데 가격을 못 읽은 건 우리가 이해하지 못하는 상황이라 신호를 지우면
  안 된다. 실측 로그에서도 `IN_STOCK` 은 전부 원가가 있었다.
- 상태: **수정완료(검증대기)** 2026-09-03

### D-284 — 마켓이 막아둔 등록을 매 배치마다 다시 두드린다

- 심각도: 중 · 리스크: 경량 · 상태: **발견** 2026-09-03
- 사실: 쿠팡 등록 중 **596건이 `BLOCKED_BY_MARKET`** 이다(판매중지·심사중). 이번 배치의 부분실패
  560건 중 **523건이 이것 하나**다. 나머지 43건은 `삭제된 상품은 변경이 불가능합니다`.
- 분류는 이미 맞다. `MarketFailureClassifier` 가 "판매중지"·"삭제된 상품"을 알아보고
  `sb_market_registration.last_sync_error` 에 제대로 기록하고 있다.
- 문제는 **아무도 그 기록을 읽지 않는다**는 것이다. `ProductMarketSyncService.syncInternal` 은
  `findByProductId` 로 전 등록을 훑고 `getLastSyncError()` 도 `getIsSynced()` 도 보지 않는다.
  그래서 막힌 596건에 매 배치마다 API 를 쏘고, 매번 같은 이유로 실패한다.
- 왜 문제인가: ① 헛 API 호출 596회/배치 ② **실패 숫자가 부풀어 진짜 문제를 가린다** — 이번 배치의
  부분실패 560건 중 실제로 조치할 수 있는 건 거의 없는데 화면은 560건이라고 말한다. [[D-269]] 가
  고치려던 바로 그 병이 다른 자리에서 재발한 셈이다.
- 수정: `BLOCKED_BY_MARKET` 과 `DELETED_ON_MARKET` 을 전송 대상에서 빼고 `skipped` 로 집계한다.
  영구 제외는 하지 않는다 — 심사는 언젠가 끝난다. `last_sync_error_at` 으로부터 **7일**이 지나면
  한 번 다시 두드려 상태를 재확인한다. `NEVER_SYNCED` 는 건너뛰지 않는다 — 첫 전송은 시도해야
  한다. `TRANSIENT_ERROR` 도 건너뛰지 않는다 — 재시도로 풀릴 수 있다.
  [[is-synced-means-present-on-market]] 의 프로브 규율과 같은 결이다.
- 상태: **수정완료(검증대기)** 2026-09-03

### D-285 — 쿠팡 오류 메시지의 HTML 이 그대로 저장되고 화면에 찍힌다

- 심각도: 낮음 · 리스크: 경량 · 상태: **수정완료(검증대기)** 2026-09-03
- 사실: 쿠팡 판매중지 응답 본문이 `<span style="font-weight: normal">…</span><br /><a href=…>` 범벅이다.
  이게 `sb_process_status.details` 에 원문 그대로 저장되고, 배치 결과 표에 **태그가 글자로 찍힌다.**
- [[D-269]] 로 만든 결과 표의 가독성을 이 한 마켓이 무너뜨린다. 화면 한 줄에 들어가야 할 사유가
  다섯 줄짜리 HTML 이 된다.
- 수정: `ProductMarketSyncService.rootMessage` 에서 정화한다. 이 한 메서드의 반환값이 DB 기록·결과
  집계·로그 **세 곳 모두**로 흘러가므로 여기서 없애야 전부가 덕을 본다.
  태그 제거는 `</?[a-zA-Z][^>]*>` 로 한정했다 — `<[^>]*>` 전체 매치는 `A < B` 같은 부등호가
...
### D-289 — 아이허브 응답 파싱이 실패하면 아직도 "품절"로 단정한다

- 심각도: 중 · 리스크: 경량 · 상태: **발견** 2026-09-04
- [[D-239]] 복구 확인 중 발견. D-239 는 세 실패 경로를 "모르면 모른다"로 고쳤는데
  **네 번째 경로가 남아 있다.** `IherbScraperClient.parseResponse` 의 catch-all:

```java
} catch (Exception e) {
    log.error("아이허브 응답 파싱 실패", e);
    return new StockCheckResult(StockStatus.OUT_OF_STOCK, null, 0, null);  // ← 품절 단정
}
```

- 200 응답인데 파싱이 깨지는 경우라 발생 확률은 낮다. 그러나 **D-239 가 세운 원칙을 이 경로만
  어기고 있다.**
- **[[D-283]] 배포로 위험이 커졌다.** 전에는 잘못된 품절 판정이 원가 예외로 막혀 마켓까지 못
  갔는데, 이제는 "품절 + 원가 없음"이 정상 경로가 되어 **재고 0 이 4개 마켓으로 곧장 나간다.**
- 수정: 다른 세 경로와 같이 예외를 던진다. 예외 메시지에 응답 앞부분(120자)을 실어 무엇을 못
  읽었는지 남긴다.
- **테스트를 쓰다 두 번째 구멍을 찾았다** — 빈 응답에 Jackson 은 예외를 던지지 않고
  `MissingNode` 를 돌려준다. 그래서 **200 에 빈 본문이 오면 조용히 품절이 됐다.**
  `isObject()` 가 아니면 명시적으로 막는다.
- 파싱 실패는 재시도 루프 안에서 3회 재시도된 뒤 전파된다(재시도가 파싱을 고치진 않지만
  드물고 무해하다). 근본 원인 메시지는 그대로 살아 화면·로그에 나온다.
- 상태: **수정완료(검증대기)** 2026-09-04

### D-290 — 소싱처별 대상 산출이 삭제된 상품을 포함한다

- 심각도: 중 · 리스크: 경량 · 상태: **발견** 2026-09-04
- [[D-240]] 스케줄 활성화 판단 중 발견. `ProductRepository.findByVendor` 에 `deletedAt` 필터가 없다:

```java
@Query("SELECT p FROM Product p WHERE p.sourcingInfo.vendor = :vendor")
List<Product> findByVendor(@Param("vendor") VendorType vendor);
```

  `BatchPriceStockService.getProductIdsByVendor` 가 이걸 그대로 쓴다 — **소싱처별 배치의 대상
  산출 경로다.**
- 실측 삭제 상품 291건: OCD 136 · TES 98 · VTB 35 · COK 14 · FTN 7 · IHB 1.
...
### D-261 브랜드 표기 불일치 연구 (2026-09-04, 사용자 지적)

사용자: "네이처스웨이 네이쳐스웨이 한 글자 다르다고 제대로 처리되지 못하면 억울해."

**실측 — 얼마나 자주 생기나.** 쿠팡이 실제로 매칭한 9건과 아이허브 표기의 편집거리:

| 쿠팡 | 아이허브 | 거리 |
|---|---|---|
| 네이**쳐**스웨이 | 네이**처**스웨이 | **1** |
| 나머지 8건(네이처스플러스·노르딕내추럴스·엔자이메디카·오가닉인디아·줄리안베이커리·퓨어인디안푸즈·콤비타·21세기센트리) | 동일 | 0 |

**9건 중 8건은 완전 일치.** 억울한 사례는 실재하지만 소수다.

**규모 — 사람이 볼 수 있다.** 백필 대상 2,893건의 **서로 다른 브랜드는 439개**다.
그리고 집중도가 높다: `Frontier` 140 · `Natural` 113 · `Nature's` 94 · `NOW` 93 · `Fortnum` 89.
**브랜드 하나를 잘못 정하면 100건 넘게 한꺼번에 틀린다 — 정밀도가 재현율보다 중요하다.**

**줄이는 방법 — 세 겹**

1. **`GET /brands/enrolled` (미사용 API)** — 우리가 이미 등록한 브랜드의 쿠팡 공식 문자열 전체
   목록. 호출 1회. **여기 있는 브랜드는 추측도 유사도도 필요 없다.** 가장 확실한 층이다.
2. **검색 후보 중에서 고르기** — 검색은 후보를 최대 10개 주는데 지금은 정확일치 하나만 보고
   나머지를 버린다. 정확일치가 없을 때 **거리 1 이내가 정확히 하나뿐이면** 채택하고, 둘 이상
   비슷하거나 아무것도 안 비슷하면 쓰지 않는다. **우리가 브랜드명을 지어내지 않고 쿠팡이 제시한
   후보 중에서만 고른다**는 것이 핵심이다.
3. **나머지는 사람이 본다** — 439개 중 1·2층에서 안 풀린 것만 남는다. 상품 2,893건이 아니라
   브랜드 수십 개를 보는 일이다.

**지금까지 한 것**: 검색 API 연동 + 세 상태 구분(`MATCHED`/`NOT_REGISTERED`/`LOOKUP_FAILED`).
조회 실패는 캐시하지 않고 기존 값을 유지한다 — 429 한 번에 `Nature's Way` 94건이 통째로
잘못될 수 있었다.

- 다음: 1층(`enrolled`)부터. 호출 한 번으로 439개 중 몇 개가 즉시 풀리는지 보고 그 수로
  2층 임계를 정한다.

### D-261 라이브 검증 — 매칭 규칙이 틀렸다 (2026-09-04)

**단건 프로브로 확인했다. 2,893건을 돌리기 전에 잡았다.**

**① 쿠팡 검색 API 는 완벽히 동작한다.** `Enzymedica` 로 검색하니
`{"totalCount":1,"items":[{"brandId":"KR-50988","brandName":"엔자이메디카"}]}` 가 온다.
**쿠팡이 교차언어 매칭을 이미 해준다.**

**② 그런데 우리 규칙이 그 답을 버린다.** "검색어와 결과가 정규화 후 같아야 한다"고 요구하니
`enzymedica ≠ 엔자이메디카` 라 버렸다. 프로브 없이 대량 실행했으면 **전 건이 "쿠팡 미등록"으로
채워졌을 것이다.**

**③ 더 나쁜 것 — 정확일치는 틀린 항목을 고른다.** 후보 목록 실측:

| 검색어 | 쿠팡 후보(순서대로) | 정확일치가 고른 것 | 실제 정답 |
|---|---|---|---|
| `Nature's Way` | **네이쳐스웨이** · 네이처스웨이 · 네이쳐스랩 … | 없음(미등록 판정) | 네이쳐스웨이 |
| `네이처스웨이` | **네이쳐스웨이** · 네이처스웨이 · … | `네이처스웨이`(2위) | 네이쳐스웨이(1위) |
| `Now` | 나우출판사 · **now** · 나우푸드 … | `now`(2위) | 나우푸드 |
| `Natural` | 네추럴엠 · 네츄럴오 · **Natural** … | `Natural`(3위) | (잘린 검색어라 무의미) |
| `Natural Factors` | **네추럴팩터스** · 네추럴팩토리 … | 없음 | 네추럴팩터스(1위) |
| `Thorne Research` | **쏜리서치** · 리서치웨이 … | 없음 | 쏜리서치(1위) |

**쿠팡은 관련도순으로 정렬한다. 온전한 브랜드명으로 검색하면 1위가 정답이다.**
정확일치는 그 1위를 버리고 엉뚱한 2·3위를 집는다 — `Now` → `now`(출판사가 아닌 무언가),
`네이처스웨이` → 쿠팡이 실제로 쓰는 `네이쳐스웨이` 가 아닌 다른 항목.

**④ 사용자가 걱정한 "한 글자 차이"의 정체.** `네이쳐스웨이` 와 `네이처스웨이` 는
**쿠팡에 둘 다 존재하는 별개 브랜드 항목**이다. 표기 흔들림이 아니라 중복 등록이다.
우리 상품에 붙은 것은 `네이쳐스웨이`(1위). 정확일치는 `네이처스웨이` 를 골라 **틀린 쪽에 붙인다.**

**⑤ 잘린 첫 단어로 검색하면 위험하다.** `Frontier` → 1위가 `프론티얼`(우리 상품은 Frontier
Co-op). `Natural` → 상품 113건이 걸린 검색어인데 후보가 전부 무관하다.
**백필은 잘린 값이 아니라 크롤로 얻은 온전한 브랜드로 검색하므로 이 경우는 피한다.**

**수정 방향 — 정확일치를 버리고 순위를 쓴다**
1. **`GET /brands/enrolled` 로 우리가 등록한 브랜드 집합을 먼저 확보한다.** 1위 후보가 그 집합에
   있으면 **확정**이다. 추측이 없다.
2. enrolled 에 없으면 **1위 후보를 제안**하되 자동 적용하지 않고 사람이 볼 목록에 넣는다.
   439개 브랜드 규모라 감당된다.
3. 정확일치 규칙은 **제거한다.** 위 표대로 오히려 틀린 항목을 고른다.

- 상태: **매칭 규칙 재설계 필요** 2026-09-04

### D-291 — JSON-LD 스크래퍼가 브랜드를 안 꺼낸다 (OCD·COK·TES)

- 심각도: 중 · 리스크: 경량 · 상태: **발견** 2026-09-04
- [[D-261]] 브랜드 백필 실행 중 드러났다. 소싱처별 결과가 완전히 갈렸다:
...
- 원인: `scraper/scrapers/jsonld.py` 가 JSON-LD `Product` 를 파싱하면서 **`name`·`sku`·`weight` 만
  꺼내고 `brand` 를 안 꺼낸다.** `OcadoScraper`(OCD) · `TescoScraper`(TES) · `CostcoUkScraper`(COK)
  가 모두 이 클래스를 쓴다.
- **실측으로 확인했다 — 페이지에는 브랜드가 있다.** Costco 상품 페이지의 JSON-LD:
  `{"@type":"product","brand":{"@type":"Organization","name":"Marmite"},...}`
  `brand.name` 이 그대로 들어 있다. 꺼내기만 하면 된다.
- 참고: `vitabiotics.py:145` 는 이미 `brandKo=payload.get("vendor")` 로 채우고 `iherb.py:422` 는
  `data-brand-name` 을 쓴다. **JSON-LD 계열만 빠졌다.**
- 수정 방향: `jsonld.py` 의 `scrape()` 반환에 `brandKo` 를 더한다. `brand` 는 문자열일 수도
  객체(`{"name": ...}`)일 수도 있으니 둘 다 받아야 한다.

### D-292 — FTN 386건은 단일 브랜드라 검색 없이 확정할 수 있다

- 심각도: 낮음 · 리스크: 경량 · 상태: **발견** 2026-09-04
- FTN(포트넘앤메이슨)은 **소싱처 자체가 브랜드**다. 상품명이 브랜드를 반복하지 않아
  (386건 중 `Fortnum` 으로 시작하는 것은 115건뿐) 첫 단어 추출이 특히 무의미했다.
  `fortnum.py` 는 `brandKo` 를 채우지 않는다.
- **쿠팡에 있다 — 실측 확인.** 네 검색어가 모두 `포트넘앤메이슨` 을 1위로 준다.
  `포트넘앤메이슨` 으로 검색하면 후보가 **그것 하나뿐**이다.
- 수정 방향: 크롤에서 얻을 것이 없으므로 소싱처 단일 브랜드 매핑을 상수로 두는 편이 정직하다.

### D-293 — @EnableCaching 이 없어 @Cacheable 이 전부 무효였다

- 심각도: 경량(성능) · 리스크: 경량 · 상태: **수정완료(검증대기)** 2026-09-05
- [[D-261]] 쿠팡 브랜드 조회 작업 중 서브에이전트가 발견해 후보로 남긴 것을 정식 등재한다.
- 사실: 프로젝트 전체에 `@EnableCaching`/`CacheManager` 가 **한 곳도 없다.** `@Cacheable` 은
  `CoupangMetaService.getCategoryMeta`(카테고리 메타) 한 곳뿐인데, 활성화 없이는 조용히
  무시된다 — **발행마다 쿠팡 카테고리 메타 API 를 다시 불렀다.** 로그의
  "캐시 미스, API 호출" 이 항상 찍히는 이유였다.
- 수정: `ApiApplication` 에 `@EnableCaching` 추가. Spring Boot 기본
  `ConcurrentMapCacheManager` 가 자동 구성된다. 리플렉션 회귀
  (`ApiApplicationCachingTest`)로 고정 — 이 애노테이션이 빠지는 실수는 컴파일도 테스트도
  잡지 못하고 조용히 성능만 잃는다.
- 브랜드 조회([[D-261]])는 자체 `ConcurrentHashMap` 캐시라 이 결함과 무관하게 이미 동작했다.

### D-261 브랜드 백필 완주 (2026-09-05)

**잘림 2,893건 → 잔여 29건. 실패 0.**

| 배치 | 대상 | 쿠팡확정 | 비고 |
|---|---|---|---|
| 1차 `b89e2538` | 2,883 (전 소싱처) | 2,089 | FTN·OCD·COK 는 크롤이 브랜드를 못 줘 792 스킵 |
| [[D-291]]·[[D-292]] 수정 배포 | — | — | JSON-LD brand 추출 + FTN 상수 + 라우팅 |
| 2차 `57d1d3c9` | 783 (스킵분) | 751 | FTN 376 전량 `포트넘앤메이슨`(후보 1개) |

잔여 29건: COK 2(JSON-LD 에 Product 없음 — [[D-247]]) · VTB 8(vitabiotics 상세가 brandKo
미제공) · OCD 17(단일어 브랜드가 첫 단어와 같음 — Marmite 류, **정상**) · 기타 2.

**검토 대상(오탐 의심) — 후보 기록에서 걸러냄, 총 ~20건 규모:**

| 영문 원문 | 박힌 값 | 의심 이유 |
|---|---|---|
| Zhou Nutrition (3건) | `산` | 무관 브랜드 확실 |
| Booja Booja (1건) | `부자오빠` | 음차 어색 — 쿠팡 무관 브랜드 가능성 |
| Mulino Bianco (2건) | `바릴라` | 모회사 매칭 — 상품 브랜드는 물리노 비앙코 |
| Lake Avenue (2건) | `캘리포니아골드뉴트리션` | 크롤 brandKo 오염 의심 |
| Bluebonnet Nutrition (1차 파일럿 1건) | `블루뉴트리션` | 부분 단어 매칭 |
| Frontier (140건) | `프론티어벌크` | 다건이라 **맞는지 우선 확인 필요** — 틀리면 140건 |

모든 선택은 `sb_process_status.message` 에 후보 목록과 함께 남아 있어 사후 교정 가능하다.
마켓 전송은 하지 않았다 — [[D-260]] seller gating 해소가 선행이다.

### D-295 — 카페24 등록 페이로드의 스키마 불일치 셋 (D-294 조사 중 발견)

- 심각도: 중 · 리스크: 표준 · 상태: **라이브 완결** 2026-09-05 — 수동 등록 1건(1149→product_no 22020)으로 brand_code(B00000NG 자동생성)·product_weight(0.60kg)·태그 7종 실반영을 GET 대조로 확인 후 테스트 등록 정리(마켓 삭제+등록행 제거). 원산지는 배지 경로에 originPlace 미탑재라 이 경로에선 안 실림(소싱 초안 등록 경로 전용) — 리졸버 실응답 파싱은 프로브로 기검증
- ① `brand`(자유 텍스트) 전송 — 스키마는 `brand_code`(사전 등록 코드). 무시되고 있을 가능성.
  `Cafe24MarketClient.java:77-78` 확인.
- ② `origin_place_value` 단독 전송 — 문서상 원산지가 "기타(1800)"일 때만 유효한 필드인데
  `origin_classification`/`origin_place_no` 없이 보낸다(`:82-84`).
- ③ `product_weight`·`product_tag` — 카페24에 구조화 필드가 **있는데** 등록·수정 어디에도
  안 쓴다. 4마켓 중 유일하게 필드가 있는 마켓에서만 미배선.
- 라이브 확인(실제로 무시되는지)이 선행. 확인은 마켓 GET 대조로 가능하다.
- **라이브 실측(2026-09-05)**: `GET /admin/origin` 응답 래퍼 키는 `origin`(관대 파싱이 커버),
  `origin_place_name` 은 `["아메리카","미국"]` 같은 **배열** — 문자열 비교는 전부 기타로
  떨어지는 실결함이라 마지막 요소 완전일치로 교정(Red→Green). 미국=358/T 확인.
  잔여 라이브 대조: 신규 등록 1건의 brand_code·무게·태그 반영.
- **수정(2026-09-05)**: ① 등록도 `Cafe24BrandCodeResolver` 경유 `brand_code`(해석 실패는 브랜드
  없이 등록 진행), ② 신규 `Cafe24OriginResolver` 가 `GET /admin/origin` 이름 완전일치로
  `origin_classification`+`origin_place_no` 를 정하고, 못 찾으면 문서가 정한 기타 조합
  (`E`+`1800`+30자 절단 `origin_place_value`)으로 떨어진다, ③ `product_weight`(kg)는 등록
  페이로드에, 검색키워드는 등록 직후 `POST /admin/products/{no}/tags` 로 전송(실패해도 등록 유지).
  인프라 442테스트 통과. **라이브 대조 남음**: 원산지 응답 래퍼 키(`origins`/`origin`)와
  `brand_code`·`product_weight`·태그 실제 반영 여부.

### D-296 — 11번가 상세설명 수정이 전체 XML 라운드트립을 쓴다

- 심각도: 낮음 · 리스크: 경량 · 상태: **검증통과(라이브 대조 대기)** 2026-09-05
- 11번가에는 상세설명 전용 부분수정 API(`POST /rest/prodservices/updateProductDetailCont/{prdNo}`)가
  있는데, `ElevenstMarketClient.syncImagesAndHtml` 은 전체 XML GET→치환→PUT 을 쓴다.
- 동작은 하지만 전체 덮어쓰기라 **다른 필드를 실수로 건드릴 위험이 구조적으로 더 크다**
  (11번가 문서: "기존 데이터는 사라지고 수정되는 정보로 교체").
- [[D-294]] 1단계에서 전용 API 로 갈아탈 것.
- **라이브 실측(2026-09-05 배포 후)**: 동일 상세설명 재전송으로 봉투 확인 — 성공은
  `<ProductDetailCont><resultCode>000</resultCode>` 이다(유추했던 `<Product/>` 아님). 판정을
  `resultCode 000` 정확 매칭으로 교정(부분문자열 판정 금지 원칙), 픽스처 6곳 실측 봉투로 갱신.
- **수정(2026-09-05)**: `ElevenstMarketClient.syncImagesAndHtml` 을 상세설명(전용 POST)·이미지(전체 XML)
  두 경로로 분기. 상세설명만 바꾸면 전체 라운드트립을 아예 안 탄다. 이미지 때문에 전체 PUT 을 타도
  그 전문에서 `<htmlDetail>` 치환을 뺐다. Red 7건 → Green, 11번가 인프라 테스트 42건 전부 통과.
  **라이브 미검증** — 성공 판정(성공 루트 `Product` / 오류 루트 `Products`)이 PDF 예제에서만 온 유추라
  실호출 1건 확인이 필요하다.

### D-297 — 판매가 저장이 이중 기록된다

- 심각도: 낮음(무해) · 리스크: 경량 · 상태: **발견** 2026-09-05
- 상세 모달 저장 시 `updatePriceStock`(마켓 반영 O) 후 같은 값이 `updateProduct` 커맨드에도
  실려 DB 에 한 번 더 쓰인다. 결과는 같지만 호출 중복. [[D-294]] 2단계(모달 개편)에서 정리.

### D-294 브랜드 일괄 마켓 반영 완주 (2026-09-05)

배치 `0ce655a6`(2,829건): **성공 2,344 · 부분실패 485 · 실패 0.**
마켓별 반영: **카페24 2,816 · 스토어 2,816 · 11번가 1,681 = 총 7,313건.**
백필 한글 브랜드가 3마켓에 실제 반영됐다. 쿠팡은 설계대로 제외(심사 트리거).

부분실패 485는 전부 해명됐다: ① 11번가 "존재하지 않는 상품" 385 — 마켓에서 이미 삭제됐는데
장부에 살아있던 등록이 드러나 `DELETED_ON_MARKET` 자동 교정(64→485건, 좋은 부수효과)
② 고시 불일치 82([[D-298]]) ③ 금칙어 15([[D-299]]) ④ 코드 부재 3.

### D-294 2단계 라이브 검증 통과 — 상세 모달 저장/마켓반영 분리 (2026-09-05)

상품 2628(231108IHB023)로 전 흐름 검증: 제조사 변경 → 저장(모달 유지) →
반영 패널 "변경된 필드: 제조사" 표시 → 스/11 자동 체크·**카페24 자동 비활성(변경 필드
미지원, 건너뜀)**·쿠팡 잠김 → 반영 실행 → 결과 행 "N스토어 ✓ 반영됨 / 11번가 ✓ 반영됨".
서버 교차 확인: 스토어 PUT 에 `manufacturerName=Now Foods, brandName=나우푸드`(변경 필드만
치환, consumptionDateText 보정 동반), 11번가 XML 라운드트립 성공, `sb_process_status`
55b06a87 성공2/스킵0/실패0. 검증 중 목격한 "저장 후 모달 닫힘"은 결함이 아니라 브라우저
자동화의 좌표 클릭이 마스크에 떨어진 조작 오류였다 — JS 직접 클릭으로 재검하니 설계대로.

### D-298 — 11번가 옛상품의 고시정보가 현행 규격과 안 맞아 모든 수정이 거부된다

- 심각도: 중 · 리스크: 표준 · 상태: **라이브 완결** 2026-09-05 — 공식 코드표(건기식 891032·13항목) 반영 후 84건(고시 82+금칙어 교집합 2) 전원 타깃 재시도로 11번가 반영 성공(재PUT 84회 로그 확인)
- 82건이 `고시유형코드에 해당하는 고시 항목 개수가 일치하지 않습니다` 로 거부.
- ~~GET 으로 받은 기존 XML 을 그대로 되돌려도 거부되므로 상품 데이터 자체가 낡은 것~~
  → **반증됨(아래 실측). GET 에는 고시 블록이 애초에 없다.**
- ~~수정 방향: PUT 전에 고시 블록을 현행 유형코드 규격으로 재구성하는 보정~~
  → **수정됨: 재구성이 아니라 생성·주입.** 스토어 `consumptionDateText`([[barcode-market-push]])와
  "옛상품 함정"이라는 성격은 같지만, 보정 대상이 응답에 없다는 점이 다르다.
- **원인 재확정(2026-09-05 실측)**: 실패상품 3168912619 · 성공상품 6465267349 의 GET 원문을
  대조하니 **둘 다 `ProductNotification` 블록이 아예 없다**(태그 50개 시퀀스 동일, 카테고리 동일 1127358).
  "받은 낡은 블록을 되돌려서 거부된다"는 최초 가설은 틀렸다. 이 GET 은 고시를 안 싣고 우리 PUT 도
  늘 고시 없이 나갔으며 1,681건이 그렇게 통과했다. 거부되는 82건은 **마켓측 저장 고시 스냅샷이
  낡은 것**으로 GET 으로는 관찰 불가.
- **수정 방향 확정**: 받은 블록 보정이 아니라 **현행 규격 블록을 생성해 PUT 에 실어 마켓의 낡은
  저장값을 덮어쓴다.**
- **수정(2026-09-05)**: `ElevenstProductNotice` 신설 — 유형코드·항목코드 상수표 + 규격 블록
  빌더(`<type>` + `<item><code>/<name>`) + 주입기. `injectElevenstRequiredFields` 말미에 배선하고
  `buildNotificationXml` 도 같은 표를 쓰도록 교정해 **파생 결함(옛 `pdNo`/`ProductNotificationItem`
  형식)을 함께 해소**했다. Red 10건 → Green, 11번가 53건·전 모듈 1,954건 통과.
- ~~코드표는 아직 비어 있다~~ → **확보·반영 완료(2026-09-05).** 사용자가 11번가 공식
  `11번가_2023상품정보제공고시_전체목록.xlsx` 를 제공했고, 해당 2유형을 추출해 `SPECS` 를 채웠다:
  **건강기능식품 = 유형코드 891032 / 13항목**, **가공식품 = 유형코드 891031 / 11항목.**
  라벨에 곡선따옴표(U+201C/D)·모서리괄호(U+FF62/3)가 섞여 있어 전사 오류를 막으려고
  TSV 에서 Java 소스를 생성했고, 생성 결과가 원본과 완전 일치함을 재대조로 확인했다(24항목 100%).
- **신규등록 경로 매핑**: 기존 `noticeFields` 키를 새 항목코드에 의미 대응으로 연결했다
  (`expirationDate`→소비기한 및 보관방법, `nutrition`→영양정보,
  `intakeMethod`→섭취량·섭취방법 및 섭취 시 주의사항 및 부작용 가능성 등).
  대응 없는 신항목(**기능정보**, **소비자안전을 위한 주의사항**)은 플레이스홀더,
  **질병의 예방…(23759747)** 은 고정 문구가 기본값으로 들어간다. 매핑 앵커는 라벨이 아니라
  **항목코드**라 라벨 문구가 바뀌어도 안 깨진다.
- **13항목 전원 필수 여부는 미확정** — '해당 경우에 한함' 단서 항목이 있으나 개수 불일치 결함의
  안전측으로 **13개 전부 싣는다.**
- **주입 범위 판정(리더, 2026-09-05): 타깃 재시도 채택.** 무조건 주입은 기각됐다 — 정상 1,681건의
  마켓 저장 고시(실문구일 수 있음)를 플레이스홀더로 덮는 것은 표시 품질 저하이기 때문이다.
  배선: 전체 XML PUT 두 경로를 공통 `putProductXml` 로 모으고, 응답에
  `고시유형코드에 해당하는 고시 항목 개수` 가 있을 때만 같은 XML 에 고시를 주입해 **1회 재PUT**.
  재PUT 도 실패하면 **원래 거부 응답**을 그대로 던진다(가짜성공 금지). 첫 PUT 이 성공하면
  고시는 아예 안 실린다 → **1,681건 행위 불변**(테스트 `doesNotInjectNoticeOnSuccessfulPut`·
  `noRetryWhenFirstPutSucceeds` 로 고정).
- **트레이드오프(의도적)**: 재시도 주입 값은 전부 `상세설명 참조` 플레이스홀더다. DB 에 옛 상품의
  실제 고시 데이터가 없기 때문. **82건은 마켓 저장 고시 문구가 플레이스홀더로 대체된다.**
  82건은 현재 모든 수정이 막힌 상태라 "수정 가능 상태 복구"를 우선한 선택이다. 실값 공급이 정공법.
- **한계: 주입 유형이 건기식 고정** — 카테고리→유형(건기식/가공식품) 매핑이 없어 고정 배선했다.
  리더 판정으로 현 범위 유지(우리 11번가 상품은 전부 건기식/보충제 카테고리). 가공식품 상품이
  섞이면 그때 매핑이 필요하다. 가공식품(891031) 표는 이미 채워져 있어 매핑만 붙이면 된다.
- **남은 것: 라이브 1건 검증.** 82건 중 하나를 실제 수정해 재시도가 통하는지(=13항목이 맞는지)
  확인해야 최종 확정이다. 코드 경로는 전 4모듈 2,044건 0실패.

### D-299 — 11번가 전체 라운드트립이 안 바꾼 필드까지 재검사받게 한다 (금칙어 실증)

- 심각도: 중 · 리스크: 경량 · 상태: **완결** 2026-09-05 — 금칙어 15건(Solaray 9·Real Mushrooms 5·Wholesome 1·Aloe Vera 1) 상품명 한글 교정 후 전건 11번가 반영 성공. 구조 위험은 D-296 분리+D-298 타깃 재시도로 축소
- 브랜드만 바꿨는데 15건이 `상품명에 금칙어 [Solaray]`/`[Real Mushrooms]` 로 거부됐다.
  등록 후 11번가 금칙어 정책이 바뀐 상품들이고, **전체 XML 재제출이라 우리가 건드리지 않은
  상품명까지 재검사**받는다 — [[D-296]] 이 지적한 구조적 위험의 실증.
- 수정 방향: [[D-296]](전용 부분수정 API 전환)과 묶어 처리. 상품명 금칙어 자체는 해당 상품의
  상품명 교정(브랜드 영문 제거)으로도 풀 수 있다.
- **부분수정(2026-09-05)**: [[D-296]] 으로 **상세설명 경로의 노출만** 제거됐다.
...
- 심각도: P2(게이트 차단) · 리스크 등급: **중대**(마켓 쓰기 가드) · 상태: 검증통과 — 6건 전부 해소. 보류 1건은 사용자 규칙 확정 후 어댑터 수정 → 검증자 반려(F-1 오버셀링 노출·F-3 픽스처 오염) → 재수정·재검증 PASS → V-1(신규 분기 가드 미고정) 보강. 최종 게이트 `./gradlew test --rerun-tasks` **3285건 failures 0 · errors 0**(core 1803 · infrastructure 1014 · api 386 · worker 82)
...
- 이력: 2026-09-19 검증 반려 2건 → 수정 완료(tdd-fixer). **F-1(과허용, 회귀)**: 레거시 조기 반환이 본상품 `selling=T`인 품절 상품의 **판매중지까지** 생략해 오버셀링을 노출했다(수정 전 HEAD는 `selling=F`를 보냈으므로 회귀). 리더 판정 — 사용자 규칙 ①의 범위는 "가격·수량"이고 `selling=F`는 그 어느 쪽도 아니며, `use_inventory=F`는 "재고관리 안 함"이라 수량과 무관하게 계속 팔린다. 운영 DB로 규모 측정 시도했으나 `sb_market_registration.market_detailed_info` 3186건 중 `use_inventory` 키 보유 행 0건으로 측정 불가(라이브에서만 보이는 플래그) → 보수적 조치. 조치: 레거시 조기 반환을 본상품 판매상태로 분기 — 이미 `selling=F`면 PUT 0회 유지, 아직 `selling=T`면 본상품 `{"selling":"F"}` **1회만** 보내고 가격·수량 PUT은 생략하며 전송 후 재조회 본상품으로 스냅샷 구성(확인 필드 `["selling"]`), 이 분기에서 `use_inventory`는 켜지 않는다. reviewed 경로 ①은 현행 유지(PUT 0회 + `UnsupportedOperationException`) — 운영자 단건 작업이라 BLOCKED로 표면화되는 반면 레거시는 무인 배치라 아무도 보지 않는다는 비대칭이 근거. **F-3(픽스처 오염)**: `Cafe24BatchStockContractTest` 공용 픽스처가 `use_inventory:"F"`(비정상 상품)라 3개 테스트가 의도와 다른 경로를 타고 있었다 → 공용 픽스처를 `"T"`로 정상화하고 `use_inventory=F`가 실제 필요한 read 테스트 1건만 명시 오버라이드, `times(3)`·`times(2)`로 총 PUT 수 고정, `stopSellingBeforeWritingZeroInventory`의 임시 오버라이드 제거. Red 실측: `stillSellingAndSoldOutWithInventoryManagementOffStopsSellingWithoutPriceOrQuantity` 1건 적색("Wanted but not invoked: put(/admin/products/7034, {selling=F})"); 짝인 already-stopped 케이스는 수정 전후 동작이 같아 처음부터 그린인 **회귀 가드**임을 명시. 조기 반환이 좁아지며 `MarketStockContractTest`·`MarketPriceContractTest`의 legacy stand-down 테스트 2건도 새 계약으로 재정렬(price 쪽은 `never().put()` → `times(1)`+바디 정확일치로 강화). 게이트: `./gradlew test --rerun-tasks` BUILD SUCCESSFUL 4m29s — core 1803 · infrastructure 1011 · api 386, 전부 failures 0 · errors 0. **F-2(레거시 stand-down이 `ProductMarketSyncService`의 `synced`·"성공" 로그를 타 아무것도 보내지 않은 실행이 성공으로 집계됨)·F-4(reviewed 경로가 `readStockQuantity` 직후 inventories를 1회 더 GET — 쓰기 1건당 레이트리밋 비용 소폭 증가)는 관찰로만 기록, 미수정.** 상세 `_workspace/fixes/D-304.md`.
...
- 이력: 2026-09-19 보류 1건 해소(tdd-fixer, 수정완료·검증대기). **사용자 확정 규칙**: 카페24 품절 상품은 `use_inventory=F`로 두고, 재실행 시 여전히 품절(목표수량 0/판매불가)이면 가격·수량을 아무것도 보내지 않으며, 판매가능(수량>0)으로 바뀌면 `use_inventory=T`를 먼저 PUT한 뒤 가격·수량을 조정한다. 보류 테스트가 요구하던 `readStockQuantity(...).writable()==false`는 재입고 시 켜고 진행하는 이 규칙과 양립 불가라 폐기하고 ①(품절→PUT 0회)·②(재입고→use_inventory PUT 선행) 두 계약으로 재작성. 어댑터 `Cafe24MarketClient` 두 경로를 같은 규칙으로 수정 — reviewed `writeStockQuantity`는 `beforeWrite` 실행 전에 `UnsupportedOperationException`(호출자 `MarketStockSyncService`가 BLOCKED 종결로 재시도 루프 차단), 레거시 `syncVerifiedPriceStock`은 예외 없이 현재 스냅샷 반환(호출자 `ProductMarketSyncService`가 모든 예외를 recordSyncError로 적재하므로 정상 동작을 실패로 쌓지 않기 위함). use_inventory 외 가드(마켓플러스·variant selling·display_soldout·자동 판매재개 보류)는 전부 유지. 옛 규칙을 담고 있던 기존 테스트 2건(`Cafe24BatchStockContractTest.stopSellingBeforeWritingZeroInventory`, `Cafe24MarketClientSoldOutTest.inventorySettingsAreNotEnabledAsSideEffect`)은 의도별로 분리·조정. Red 실행 로그로 신규 5건 전부 적색 확인 후 Green. 게이트: `:infrastructure:test` 1010건 0실패(XML 집계 failures 0·errors 0), `:core:test :api:test` BUILD SUCCESSFUL. 상세 `_workspace/fixes/D-304.md`.

- 이력: 2026-09-19 재검증 PASS(verifier-d304, 판정서 `_workspace/verify/D-304_final.md`) — F-1 조기 반환 분기의 재조회 강도가 성공 경로와 동일함(재조회·`requireNativeWrite`·식별자 재확인·상태 확증), confirmed 필드 `["selling"]` 단독, 변형 4종으로 적색 14건 확인. **V-1(검증자 신규 지적, 리더 직접 보강)**: stand-down 분기의 재조회 가드 3개를 전부 삭제해도 `:infrastructure:test`가 그린이라 계약이 비어 있었다 → `Cafe24MarketClientSoldOutTest`에 `standDownCannotConfirmWhenProductIdentityChangesAfterStoppingSale`·`standDownCannotConfirmWhenSaleIsStillOnAfterStopping`·`standDownCannotConfirmWhenProductBecomesMarketPlusLinkedAfterStopping` 3건 추가. 가드 제거 변형으로 **정확히 신규 3건만 적색**(27 tests, 3 failed) 확인 후 복원. 최종 전 모듈 게이트 3285건 0실패(infrastructure 1011→1014). 미검증으로 남긴 것: 라이브 카페24에서 `use_inventory=F` 상태 전송이 실제로 거부되는지, 해당 상품이 운영에 몇 건인지(로컬 DB에 플래그 미저장).
...
### D-313 — 배포 스크립트의 변경 판정·롤백 태그가 containerd 이미지 저장소에서 오작동

- 심각도: 높음 · 상태: 수정·라이브 검증 완료(1a859f39, 2026-09-20)
- 이 서버 Docker는 containerd 저장소라 이미지 Id가 어테스테이션을 감싼 인덱스 다이제스트이고 **캐시 빌드마다 바뀐다**. 컨테이너 `.Image` 와의 비교는 항상 "변경됨" → 3개 서비스 전부 교체 대상이 되고, 빌드 뒤에는 옛 이미지를 조회할 수 없어 롤백 태그 생성이 `No such image` 로 실패했다(deploy-v2 첫 실행, 컨테이너는 교체 전이라 무사).
- 가짜 도커가 `BUILT`/`RUNNING` 을 같은 문자열로 다뤄 102개 테스트가 이 부류를 원리적으로 못 잡았다.
- 수정: 변경 판정=`:latest` 의 `.Config`+`.RootFS` 내용 지문 vs 성공 배포 때 `~/.sbshop-deploy/<svc>.fp` 기록. 롤백 태그=빌드 전 `pending-prev` 스냅샷→교체 확정 시 `prev-시각`. 지문은 헬스 통과 뒤에만 기록. 테스트 121, 변형 8건 사망, 실서버 5회 실행(교체·no-op·recreate·rollback·push) 통과.
- 잔여: 이전 배포자(구 deploy.yml·웹훅)가 상태 파일을 안 남기던 시절의 첫 실행은 전체 교체 1회(감수).

### D-314 — 배포 스크립트 테스트 사각지대(검증자 관찰)

- 심각도: 중 · 상태: 종결(검증 PASS·실서버 확인 — f7473e81·416feb99·b9546ba5)
- 조치: 기본값을 `env -i` 별도 프로세스로 고정, prune 배선·헬스 본문·롤백 사후확인(헬스·nginx·frontend reload)·지문 기록 시점·frontend 단독 배포 시나리오 단언 추가. 테스트 121→182, 변형 34건 사망. 가짜 도커로 원리적으로 못 잡는 것(지문을 `.Config`만으로 계산하는 변형)은 실서버 실행이 유일한 근거로 남는다.
- 살아남은 변형: `tag_prev` 의 `prune_prev_tags` 호출 배선 미단언(N1), `LOCK_WAIT_SEC`·`MIN_FREE_KB`·`HEALTH_WAIT_SEC` 기본값 미검증(N2·N5·N6), 롤백 후 헬스체크 미단언(N3), 헬스 판정 문자열 미검증(N4), `UP_RC`/`set -e` 는 운영과 테스트가 다르게 동작. 지문 방식 도입 후에도 가짜 도커의 한계는 그대로.

### D-315 — 배포 스크립트 종료코드·메시지 체계(검증자 관찰)

- 심각도: 낮음~중 · 상태: 종결(검증 PASS·실서버 확인 — f7473e81·416feb99·b9546ba5)
- 조치: 종료코드 6(교체 뒤 서비스가 내려갔을 수 있음) 신설, exit 3 도 `실패:` 줄 출력, `die` 는 `$1`만 출력, `RECREATE` 글롭 미확장, 롤백은 nginx 실패해도 헬스 수행·frontend 롤백도 nginx reload, selenium 찌꺼기는 본체가 있을 때만 제거, DRY_RUN 안내, 지문 기록 실패는 경고만. 아래 추가 관찰 ①~⑥ 처리 완료. 종료코드표: 1 교체 전 실패 · 2 사용법 · 3 배치 대기 초과 · 4 기동 실패 · 5 nginx 실패 · 6 교체 뒤 검증·헬스 실패.
- 종료코드 1이 "아무것도 안 건드림"(빌드 실패)과 "서비스가 내려갔을 수 있음"(verify·헬스 실패)을 함께 뜻함. `wait_for_idle_batches || exit 3` 은 `실패:` 줄 없이 경고만 출력. 도커 원문이 `die` 를 거치지 않고 노출되는 경로 존재. 크론·이메일 수집은 배치 가드에 포함되지 않음(설계상 재시작 안전).
- 추가 관찰(검증자 1차 재검토): ① `rollback_service` 는 nginx reload 실패 시 헬스체크 없이 종료(rc=5) ② `die` 가 `$*` 로 종료코드까지 메시지에 찍음(`$1` 로 교정) ③ `for s in $RECREATE` 글롭(`RECREATE="*"` → 엉뚱한 서비스명 오류) ④ nginx·헬스 동시 실패 시 nginx 실패 정보 소실 ⑤ `--no-deps` + selenium 찌꺼기 정규식은 지우기만 하고 되살리지 않음 ⑥ DRY_RUN 은 빌드를 건너뛰어 항상 "변경 없음"이 나오므로 무변경의 증거가 아니다.

### D-316 — 교체 도중 실패 시 nginx가 옛 IP를 물고 있음(검증자 발견)
...
### D-318 — 429·쿨다운 테스트 4건이 Linux에서만 실패(Instant 나노초 정밀도)
...
### D-321 — 배포 3단계: 자동 롤백과 교체 뒤 점검 확대 (개선)

- 심각도: 개선(P3) · 상태: 종결(검증자 조건부 PASS·조건 반영, 실서버 검증 완료 — 2026-09-20)
- 무중단(블루/그린) 교체는 **하지 않기로 함**: api 는 스케줄러 26개(13개 클래스)·이메일 수집이 도는 단일 JVM 인데 프로세스 간 락은 토큰 갱신(advisory lock) 하나뿐이고 이메일 수집은 JVM 내부 `AtomicBoolean` 뿐이라, 옛·새 api 를 동시에 띄우면 이메일 중복 수집·주문 동기화·배치 중복 실행 위험이 있다. 스케줄러 리더 선출(코드 변경)이 선행돼야 하며 배포 파이프라인 범위 밖이다. 교체 시 api 약 20초 끊김은 유지.
- 구현: 교체 뒤 점검을 넓힘 — api 헬스(DB 포함) + **nginx 컨테이너 안에서 사용자 경로로 라우팅 점검**(`/sbshop-agent/api/v1/products` 가 200·301·302·401·403 이면 통과, 옛 IP 로 인한 502·504·연결 실패는 실패) + frontend 라우팅(`/sbshop-agent/` 200) + scraper 헬스(`{"ok":true}`). 실패하면(기동 실패·이미지 불일치·점검 실패) **이번 실행에서 교체한 서비스를 전부**(api 만 새 버전·frontend 만 옛 버전 같은 혼합 방지) 교체의 역순으로 직전 이미지로 되돌리고 nginx reload 후 다시 점검한다.
- 종료코드: 7 = 자동 롤백 성공(서비스 정상, 실행은 실패로 기록), 8 = 롤백도 실패(수동 `rollback` 필요). `AUTO_ROLLBACK=0` 으로 끄면 예전처럼 6. nginx reload 만 실패하면(코드 5) 라우팅 점검은 건너뛰고 롤백하지 않는다. 수동 `rollback` 도 frontend·scraper 까지 같은 점검을 한다. 롤백 성공 시 지문은 옛 값 그대로라 같은 커밋을 다시 배포하면 재시도된다.
- `IMAGE_PULL=0`: 레지스트리 대신 서버에 이미 있는 `ghcr.io/...:<태그>` 를 쓴다(오프라인 재배포·실서버 실패 주입 검증용).
- 테스트 259→319, 변형 20건 사망.
- 검증자 재검토 5(조건부 PASS) 반영: ① `wait_ok` 재시도 루프 테스트 추가(변형 Y7 사망) ② 롤백 재점검은 별도 예산 `ROLLBACK_WAIT_SEC`(기본 90초)를 쓰고 첫 실패에서 중단 — 실패한 배포가 잠금을 쥐는 시간 15분→수 분 ③ frontend 단독 배포·nginx 실패+frontend 교체 시나리오 테스트 추가. 인지 사항: 바뀌지 않은 서비스는 점검·롤백 대상이 아니다(이번 배포가 만든 문제가 아니므로), 최초 배포처럼 이전 이미지가 없으면 자동 롤백 불가(경고+코드 8). 테스트 336, 변형 25건 사망.
- 실서버 검증(2026-09-20): ① `recreate` 로 frontend·scraper·api 각각 새 점검이 오탐 없이 통과(api: 헬스 + nginx 라우팅) ② **실패 주입**: 기동은 되지만 앱이 뜨지 않는 api 이미지(`ENTRYPOINT sleep`)를 `IMAGE_PULL=0` 로 배포 → 30초 뒤 점검 실패 → 자동 롤백 → 재점검 통과, 종료코드 7, 총 53초, 롤백 후 실행 중 이미지가 시작 전과 동일·지문 불변·헬스 정상. ③ 실패 주입 결과에서 발견: 자동 롤백 완료 메시지에 수동 롤백 안내("롤백하려면 …")가 붙어 오해 소지 → 자동 롤백을 못 할 때만 붙이도록 수정.

### D-322 — 서버 정비 스크립트가 롤백용 `prev-*` 이미지를 지운다(잠복)

- 심각도: 중(롤백 보장 무력화) · 상태: 수정완료(테스트 23건·변형 13건 사망, 배포 후 확인 대기)
- 증상 없음(잠복): `ops/docker-storage-maintenance.sh` 가 `docker image prune -a --filter until=168h` 로 미사용 이미지를 매일 지운다. 이 필터는 이미지 **생성 시각** 기준이고 태그가 있어도 지우므로, 배포 간격이 긴 서비스(can-agent 이미지 13일 전, life-change 3주 전)는 배포 뒤 `prev-*` 로 남긴 이전 이미지가 다음 정비(매일 20:03 UTC)에 삭제돼 롤백·자동 롤백이 조용히 무력화된다. 스크립트 주석("Keep recent rollback images")과 실제 동작이 달랐다.
