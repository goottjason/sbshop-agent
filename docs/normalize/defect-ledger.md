...
- **코드표는 아직 비어 있고, 비어 있는 동안 주입은 무동작이다.** 항목 **개수**가 결함의 본체라
  개수·코드·라벨을 추측해 박으면 같은 오류를 재생산한다. 그래서 전부 표에서 오는 값으로 두고
  표를 비운 채 출하했다. 필요한 것은 여전히 **셀러오피스 고시 팝업의 유형별 항목 표**(건기식·가공식품).
