...
- 수정 방향(미착수): 진입 시 기존 식별자로 **마켓 단건 조회**를 먼저 한다(쿠팡 `GET seller-products/{id}`, 스토어 `GET origin-products/{no}` — 둘 다 구현되어 있다). 살아 있으면 게시 대신 재게시 경로로 보내거나 명시적으로 거부. 최소 조치는 `is_synced=true`+식별자 보유 시 기본 거부하고 강제 재등록은 별도 플래그로만 허용.
- 상태: 발견
...
- 수정 방향(미착수): `sb_market_registration.unsync_reason` 신설(`DELETED_ON_MARKET`/`VALIDATION_FAILED`/`TRANSIENT_ERROR`/`NEVER_SYNCED`, nullable). catch 에서 마켓 응답 문구로 분류해 기록한다 — 쿠팡 `"이미 삭제된 상품입니다"`는 확정 신호. **스키마 변경이라 수동 DDL 선행.** [[D-222]]와 같은 작업이므로 함께 처리한다.
- 상태: 발견
...
- 수정 방향(미착수): 전송 루프에서 `is_synced=false`는 `SKIPPED("미동기 등록 — 마켓에 없음")`로 건너뛴다.
- 상태: 발견
...
- 수정 방향(미착수): 식별자를 먼저 별도 적재한 뒤 등록행을 갱신하거나, PENDING 행을 마켓 조회로 화해시키는 복구 배치를 둔다.
- 상태: 발견
