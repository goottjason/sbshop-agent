...
| 11번가 | 6 | 거부 | `해당 배송번호의 주문상태가 이미 변경 되었습니다. 변경된 상태 : 배송중` |
...
- 드러난 하위 원인 2가지: ① `104119 택배사코드 확인` — 롯데 택배사 코드 매핑이 네이버 코드와 다르다.
  ② `9999 주문상태 및 클레임상태를 확인하세요` — 이미 발송된 주문은 `dispatch`로 송장을 못 바꾼다.
  `SmartStoreOrderAdapter`는 `updateTracking`을 오버라이드하지 않아 발송 API로 폴백한다 → **송장 수정 API가 없다.**
- 수정 방향: ① 응답의 `failProductOrderInfos`를 실패로 판정(거짓 성공 제거) ② 택배사 코드 교정
  ③ 발송 후 송장 수정용 API 경로 신설.

### D-146: 11번가 "이미 배송중" 거부가 일시 실패로 분류돼 무한 재시도 (2026-08-07)
- 심각도: P2 · 리스크 등급: 경량 · 상태: **발견**
...
- 수정 방향: 교정으로 송장이 바뀐 순간 플래그를 false로 내리고, 전송 성공 시에만 다시 true.
