...
- 수정 방향: 네이버 어댑터에서 전송 직전 10원 단위로 올림한다. 내림은 마진을 깎으므로 올림. 산정식 자체는 건드리지 않는다 — 이건 마켓 제약이지 가격 정책이 아니다.
...
- 더 나빠진다: 쿠팡은 7월 131건 → 8월 17건 → 9월 8건으로 줄었다. 같은 2일 임계가 **쿠팡에서도** 곧
  오작동한다. 상수는 그때의 물량을 가정하고 박혔고 물량은 변했다.
- 왜 고쳐야 하나: 정상 상태에 울리는 경보는 무시하도록 사람을 훈련시킨다. 진짜 단절이 왔을 때
  ([[esm-order-collection-outage]] 같은) 묻힌다. 경보가 없는 것보다 나쁘다.
- 수정 방향: 임계를 상수가 아니라 **그 마켓 자신의 평균 주문 간격의 배수**로 잡는다.
  `임계 = clamp(배수 × (관측창 / 관측창 내 주문수), 최소, 최대)`. 배수 3, 최소 2일, 최대 30일이면
  현재값은 스토어 ≈11일 · 쿠팡 ≈3.6일 · G마켓 ≈15일이 되어 지금의 조용함에는 울리지 않고
  진짜 단절에는 울린다.
