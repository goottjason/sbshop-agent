...
| # | 항목 | 내용 | 위험 | 선행 |
|---|------|------|------|------|
| 1 | **[[D-265]] 2단계** | terminal 폐기 + 120일 추적 — `isNonTerminal` 필터 제거, 추적 대상을 주문일 기준으로 전환, miss 카운터·'확인 필요' 목록 | 낮 | 1단계(완료) |
| 2 | **[[D-265]] 3단계** | 명령 경로 전환 — 로컬 낙관적 갱신 제거 + 즉시 단건 재조회 | **높음** (주문확인·발송처리 전 경로가 바뀐다) | 1단계 |
| 3 | **[[D-265]] 4단계** | 송장 대조 UI + 입력 경로 이전 — 마켓송장 메인/메일송장 보조/불일치 빨강, 쿠팡 자동 재전송, 입력을 PREPARING 으로 이동, DISPATCHED 이후 경로 제거 | 중(프론트 포함) | 3단계 |
