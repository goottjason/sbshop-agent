...
- **2단계 착수 전 필수 3건**: 레거시 행 백필 경로 부재(중복 생성 위험) · `uk_line_item_order_market_no`와 정규화기 대체값 충돌 · 미러 방향 전제(이메일·수동·Cafe24 필자를 먼저 배송 계층으로 옮겨야 함). 상세는 설계 문서와 메모리 `bundle-shipment-order-model` 참조.
- 상태: **1단계 완료** · 2단계(11번가) 착수 대기
