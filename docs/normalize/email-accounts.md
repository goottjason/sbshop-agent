...
> - 자동전달은 제목·본문을 원본대로 push → 파서(`OrderEmailParser`, 제목/본문 기반 매칭, 발신자 미검증)와 100% 호환. "Fwd:" 안 붙음.
> - ⚠️ POP '다른 계정 메일 확인'은 지연(최대 1h)이라 금지 — **반드시 자동전달(Forwarding)** 사용.
> - 중앙 인박스 혼잡 대비: 전달 시 `from:(iherb.com)` 필터 권장 + 코드 스캔 창 200→**1000** 확장.
> - **설정 env**: `EMAIL_CENTRAL_USERNAME`/`EMAIL_CENTRAL_PASSWORD`(슬롯식 `_1/_2/_3` 폐기). 전달 못 거는 직접계정은 `EMAIL_ACCOUNTS`로 추가 병합.
> - 남은 수동작업(사용자): 각 소싱 Gmail → 자동전달 설정. 완료 후 서버 재시작 시 활성.
> - 아래 §2~§5는 방향 전환 이전(계정별 앱비번) 조사 기록으로 보존.
