...
public class ProductContentController {
	private final ProductContentService content;

	public record CommitRequest(String reviewId) {
		public CommitRequest {
			ProductContentService.requireUuid(reviewId);
		}
	}

	@PostMapping("/collections")
	public ProductContentService.Collection collect(@RequestBody
	ProductContentService.CollectionRequest request, Principal actor) {
		return content.collect(request, actor.getName());
	}

	@GetMapping("/collections/{id}")
	public ProductContentService.Collection collection(@PathVariable
	String id, Principal actor) {
		return content.collection(id, actor.getName());
	}

	@GetMapping("/{productId}/history")
	public List<ProductContentService.Snapshot> history(@PathVariable
	...
	ProductContentService.ReviewRequest request, Principal actor) {
