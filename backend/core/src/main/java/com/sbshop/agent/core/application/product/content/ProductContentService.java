...
public class ProductContentService {
	...
	private final ProductContentCollectionRepository collections;
	private final ProductContentSnapshotRepository snapshots;
	private final ProductContentLaneRepository lanes;
	private final ProductContentReviewRepository reviews;
	...
	public record Selection(String snapshotId, List<Field> fields) {
		public Selection {
			...
			if (fields == null || fields.isEmpty() || fields.size() > 2 || fields.stream().anyMatch(Objects::isNull)
				...
				throw new IllegalArgumentException("반영할 이미지 또는 상세 HTML 항목을 선택하세요.");
	...
	public Collection collect(CollectionRequest request, String actor) {
		...
		lanes.findLocked("IHB").orElseThrow(() -> new IllegalStateException("콘텐츠 수집 DB 준비가 필요합니다."));
		...
		var collection = collections.save(new ProductContentCollection(UUID.randomUUID().toString(),
		...
		for (Long id : request.productIds()) {
			...
			var spec = product == null ? null : product.getProductSpec();
			var logistics = product == null ? null : product.getLogisticsInfo();
			var captured = new Captured(product == null ? new Values(List.of(), List.of(), "") : values(product),
				new Generation(product == null ? null : product.getProductName(),
					product == null ? null : product.getOriginalName(),
					logistics == null ? null : logistics.getBundleQuantity(), spec == null ? null : spec.getCapacity(),
					spec == null ? null : spec.getMeasureUnit()));
			...
			var snapshot = new ProductContentSnapshot(UUID.randomUUID().toString(), collection.getId(), id,
	...
	public ProductEditService.Review review(ReviewRequest request, String actor) {
		...
		for (Selection selection : request.items()) {
			...
			if (selection.fields().contains(Field.IMAGES) && (proposal == null || !proposal.imagesAvailable()))
				reasons.add("이미지 전체 수집·호스팅이 완료되지 않았습니다.");
			if (selection.fields().contains(Field.DETAIL_HTML) && (proposal == null || !proposal.detailAvailable()))
				reasons.add("상세정보 수집·자동 생성이 완료되지 않았습니다.");
			...
			if (!reasons.isEmpty()) {
				...
				if (selection.fields().contains(Field.IMAGES)) {
					values.set("sourceImages", mapper.valueToTree(proposal.values().sourceImages()));
					values.set("hostedImages", mapper.valueToTree(proposal.values().hostedImages()));
				}
				if (selection.fields().contains(Field.DETAIL_HTML))
					values.put("detailHtml", proposal.values().detailHtml());
				plan = planner.plan(product, values, links);
		...
		var review = reviews.save(new ProductContentReview(UUID.randomUUID().toString(), actor, now, expiresAt,
			boundedJson(rows)));
	...
	public ProductEditService.CommitResult commit(String reviewId, String actor) {
		...
		requireActor(actor);
		var review = reviews.findById(reviewId).orElseThrow(() -> new ProductEditConflictException("콘텐츠 검토 기록이 없습니다."));
		...
		for (Reviewed row : rows) {
			try {
				result.add(tx.execute(status -> {
					...
					if ("SAVED".equals(item.state()))
						snapshot.applied(row.fields().contains(Field.IMAGES),
							row.fields().contains(Field.DETAIL_HTML), Instant.now());
					return item;
				}));
				...
				log.error("콘텐츠 검토 저장 실패 review={}, product={}", reviewId, row.plan().productId(), e);
				result
	...
	private Collection collection(ProductContentCollection collection) {
	...
	private ProductContentCollection ownedCollection(String id, String actor) {
	...
	private ProductContentSnapshot ownedSnapshot(String id, String actor) {
	...
	private Snapshot view(ProductContentSnapshot snapshot) {
		...
		List<FieldView> fields = List.of(
			field(Field.IMAGES, proposal != null && proposal.imagesAvailable(), List.of("sourceImages", "hostedImages"),
				links,
				stale, snapshot.getImagesCollectedAt(), snapshot.getImagesAppliedAt()),
			field(Field.DETAIL_HTML, proposal != null && proposal.detailAvailable(), List.of("detailHtml"), links,
				stale, snapshot.getDetailCollectedAt(), snapshot.getDetailAppliedAt()));
		Instant appliedAt = snapshot.getImagesAppliedAt() == null ? snapshot.getDetailAppliedAt()
			: snapshot.getDetailAppliedAt() == null
				|| snapshot.getImagesAppliedAt().isAfter(snapshot.getDetailAppliedAt())
					? snapshot.getImagesAppliedAt() : snapshot.getDetailAppliedAt();
	...
	private FieldView field(Field field, boolean available, List<String> keys, List<MarketRegistration> links,
		...
		String reason = keys.stream().map(key -> policy.rule(key, links)).filter(rule -> !rule.editable())
			.map(ProductEditPolicy.Rule::reason).findFirst().orElse(null);
		if (stale != null)
			reason = stale;
		if (!available)
			reason = "이 항목의 수집 또는 자동 생성이 완료되지 않았습니다.";
	...
	private Proposed proposal(ProductContentSnapshot snapshot) {
		return snapshot.getProposed() == null ? null : read(snapshot.getProposed(), Proposed.class);
	}

	private Values values(Product product) {
		return new Values(product.getSourceImages() == null ? List.of() : List.copyOf(product.getSourceImages()),
			product.getHostedImages() == null ? List.of() : List.copyOf(product.getHostedImages()),
	...
	private String json(Object value) {
		try {
			...
			throw new IllegalStateException("콘텐츠 검토 기록 직렬화 실패", e);
		}
	}
	...
	private <T> T read(String value, Class<T> type) {
		try {
			...
			throw new IllegalStateException("콘텐츠 검토 기록 읽기 실패", e);
		}
	}
