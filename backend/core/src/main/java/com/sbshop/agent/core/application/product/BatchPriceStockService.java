	...
	public void crawlAndUpdatePriceStock(String batchId, List<Long> productIds,
		...
		for (Long productId : productIds) {
			try {
				...
				int bundleQty = product.getLogisticsInfo() != null && product.getLogisticsInfo().getBundleQuantity() != null
						? product.getLogisticsInfo().getBundleQuantity() : 1;
				BigDecimal salePrice = marginCalculator.calculateSalePrice(buyPrice, bundleQty, marginRate, minMarginPrice);
