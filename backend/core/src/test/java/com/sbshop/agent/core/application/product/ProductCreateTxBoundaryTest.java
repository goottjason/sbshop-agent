...
class ProductCreateTxBoundaryTest {
	...
	void allImageIo_happensBeforePersist() {
		...
		verify(imageStorageClient, Mockito.times(2)).uploadImages(any());
		verify(productWriter, Mockito.times(1)).saveAll(any());

		InOrder order = inOrder(imageStorageClient, productWriter);
		order.verify(imageStorageClient, Mockito.times(2)).uploadImages(any());
