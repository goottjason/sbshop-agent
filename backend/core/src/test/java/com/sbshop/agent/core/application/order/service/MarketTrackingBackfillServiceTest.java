...
class MarketTrackingBackfillServiceTest {
	...
	void splitsCoupangIntoSafeWindows() {
		...
		verify(coupang, org.mockito.Mockito.atLeastOnce()).syncCoupangOrders(from.capture(), to.capture());
	...
	void splitsCafe24WithinThreeMonthCap() {
		...
		ArgumentCaptor<LocalDate> from = ArgumentCaptor.forClass(LocalDate.class);
		ArgumentCaptor<LocalDate> to = ArgumentCaptor.forClass(LocalDate.class);
		verify(cafe24, org.mockito.Mockito.atLeastOnce()).syncCafe24Orders(from.capture(), to.capture());
	...
	void coversWholeRangeUpToToday() {
		...
		ArgumentCaptor<LocalDate> from = ArgumentCaptor.forClass(LocalDate.class);
		ArgumentCaptor<LocalDate> to = ArgumentCaptor.forClass(LocalDate.class);
		verify(elevenst, org.mockito.Mockito.atLeastOnce()).syncElevenstOrders(from.capture(), to.capture());
	...
	void oneMarketFailureDoesNotStopOthers() {
		doThrow(new RuntimeException("429 TOO_MANY_REQUESTS"))
			.when(coupang).syncCoupangOrders(any(), any());
	...
	void oneWindowFailureDoesNotStopRemainingWindows() {
		doThrow(new RuntimeException("일시 오류"))
			.doNothing()
			.when(smartStore).syncSmartStoreOrders(any(), any());
