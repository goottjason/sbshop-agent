...
class ProductMarketSyncPerMarketPriceTest {
	...
	void setUp() {
		service = new ProductMarketSyncService(
			...
			new MarketSalePriceResolver(new MarginCalculator(), marketFeeService,
				Mockito.mock(PricePolicyService.class), Mockito.mock(VendorPricePolicyService.class)),
	...
	void perMarketPrice_usesMarketFee() {
		...
		verify(coupangClient).syncPriceAndStock(eq("CP123"), any(), eq(67800), eq(300), eq(false), any());
		verify(storeClient).syncPriceAndStock(eq("OP99"), any(), eq(65400), eq(300), eq(false), any());
	}
