...
/**
 * D-052: 상품 그리드의 마켓별 연동코드가 쿠팡만 표시되고 스토어/11번가/카페24는 '미확인'으로만 뜨던 원인은
 * extractVendorItemId()(쿠팡 전용 vendorItemId 키)로 모든 마켓 코드를 읽던 데 있었다.
 * extractMarketCode()가 marketType별 실제 키로 분기함을 고정한다.
 */
class MarketRegistrationExtractMarketCodeTest {
	...
	void smartstore_wasNullUnderVendorItemIdLookup_nowResolves() {
		...
		// 기존 버그: 쿠팡 전용 추출은 스토어 코드를 못 읽어 null(→productId 폴백 '미확인')
		assertThat(store.extractVendorItemId()).isNull();
		// D-052 수정: 마켓별 추출은 실제 코드를 반환
