"""Exercise the actual field queue service, locking and rollback against isolated PostgreSQL 16."""
import os
import pathlib
import subprocess
import time
...
CONTAINER = "sbshop-field-check-" + uuid.uuid4().hex[:10]
...
try:
    run("docker", "run", "--rm", "--detach", "--name", CONTAINER,
        "--publish", "127.0.0.1::5432", "--env", "POSTGRES_DB=sbshop_field_check",
    ...
    env = dict(os.environ, SBSHOP_FIELD_TEST_POSTGRES_URL=f"jdbc:postgresql://127.0.0.1:{port}/sbshop_field_check")
    subprocess.run([str(BACKEND / "gradlew"), ":core:test", "--tests", "*MarketFieldSyncIntegrationTest",
